...
export default function ChatPage() {
  ...
  return (
    <div className="flex flex-col flex-1 w-full relative min-h-[100dvh]" dir="rtl">
      <div className="fixed inset-0 bg-[#F0F2F5] -z-10 dark:bg-[#0F172A]" />
            ...
            <div key={msg.id}>
              {showDateSeparator && (
                <div className="flex justify-center my-4">
                  <span className="bg-white text-gray-500 text-[10px] font-bold px-3 py-1 rounded-full shadow-sm dark:bg-gray-800 dark:text-gray-300">
                    {currentLabel}
                  </span>
                </div>
              )}
                ...
                >
                  ...
                  {hasMedia && (
                    <div className="mb-1 relative z-0 w-full" onClick={() => setFullScreenMedia({ url: msg.media_url, type: msg.media_type })}>
                      ...
                      {msg.media_type === 'file' && (
                        <a href={msg.media_url} target="_blank" rel="noopener noreferrer" className={`flex items-center gap-3 p-2 rounded-xl shadow-sm transition active:scale-95 ${isMe ? 'bg-[#D0E6FB]' : 'bg-white dark:bg-gray-800'}`}>
                          ...
                          <div className="flex flex-col text-left">
                            <span className="text-xs font-bold line-clamp-1 text-brand-dark dark:text-gray-200">{msg.file_name || 'מסמך מצורף'}</span>
                  ...
                  {msg.content && (
                    <div className={`p-3 text-sm shadow-sm relative z-0 ${hasMedia ? 'bg-white text-brand-dark dark:bg-gray-800 dark:text-white rounded-2xl w-full mt-1' : isMe ? 'bg-[#E3F2FD] text-brand-dark rounded-2xl rounded-br-sm' : 'bg-white text-brand-dark dark:bg-gray-800 dark:text-white dark:border-gray-700 rounded-2xl border border-gray-100/50 rounded-bl-sm'}`}>
      ...
      {activeMenu && (
        <div className="fixed inset-0 z-[100] flex flex-col justify-end">
          ...
          <div className="bg-white dark:bg-gray-900 w-full rounded-t-3xl pb-10 pt-5 px-4 relative z-10 animate-in slide-in-from-bottom-full shadow-[0_-10px_40px_rgba(0,0,0,0.2)]">
            ...
            <div className="flex justify-around items-start mt-6 px-2">
              <button onClick={() => { setReplyingTo(activeMenu); setActiveMenu(null); }} className="flex flex-col items-center gap-2 group transition active:scale-95">
                ...
                <span className="text-[11px] font-bold text-brand-dark dark:text-gray-200">השבה</span>
              </button>

              {activeMenu.content && (
                <button onClick={() => handleCopy(activeMenu.content)} className="flex flex-col items-center gap-2 group transition active:scale-95">
                  <div className="w-14 h-14 rounded-full bg-gray-100 dark:bg-gray-800 text-brand-dark dark:text-gray-300 flex items-center justify-center group-hover:bg-gray-200 transition">
                  ...
                  <span className="text-[11px] font-bold text-brand-dark dark:text-gray-200">העתקה</span>
                </button>
              )}
              ...
              {currentUser?.id === activeMenu.user_id && activeMenu.content && !activeMenu.media_url && (
                <button onClick={() => handleEditClick(activeMenu)} className="flex flex-col items-center gap-2 group transition active:scale-95">
                  ...
                  <span className="text-[11px] font-bold text-brand-dark dark:text-gray-200">עריכה</span>
                </button>
              )}
              ...
              {(currentUser?.id === activeMenu.user_id || currentUser?.role === 'admin') && (
                <button onClick={() => promptDelete(activeMenu.id)} className="flex flex-col items-center gap-2 group transition active:scale-95">
                  <div className="w-14 h-14 rounded-full bg-red-50 dark:bg-red-900/30 text-red-500 flex items-center justify-center group-hover:bg-red-100 transition">
      ...
      {errorMessage && (
        <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm flex justify-center items-center p-4">
          <div className="bg-white dark:bg-gray-900 w-full max-w-sm rounded-3xl p-6 shadow-2xl animate-in zoom-in-95">
            ...
            <h3 className="font-black text-lg text-center text-brand-dark dark:text-white mb-2">אופס, משהו השתבש</h3>
            <p className="text-sm text-center text-brand-gray mb-6 leading-relaxed">{errorMessage}</p>
            <button onClick={() => setErrorMessage(null)} className="w-full bg-gray-100 dark:bg-gray-800 text-brand-dark dark:text-white font-bold py-3 rounded-xl hover:bg-gray-200 transition active:scale-95">הבנתי, סגור</button>
          </div>
        </div>
      )}
      ...
      {confirmDialog.isOpen && (
        <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm flex justify-center items-center p-4">
          <div className="bg-white dark:bg-gray-900 w-full max-w-sm rounded-3xl p-6 shadow-2xl animate-in zoom-in-95">
            ...
            <h3 className="font-black text-lg text-center text-brand-dark dark:text-white mb-2">מחיקת הודעה</h3>
            <p className="text-sm text-center text-brand-gray mb-6 leading-relaxed">האם למחוק את ההודעה לצמיתות?</p>
            <div className="flex gap-3">
              <button onClick={() => setConfirmDialog({ isOpen: false, msgId: null })} className="flex-1 bg-gray-100 dark:bg-gray-800 text-brand-dark dark:text-white font-bold py-3 rounded-xl hover:bg-gray-200 transition active:scale-95">ביטול</button>
      ...
      <div className="fixed bottom-0 left-0 w-full flex justify-center z-50 pointer-events-none pb-4 pt-10">
        <div className="w-full max-w-md px-4 pointer-events-auto relative">
          ...
          {showEmoji && (
            <div className="absolute bottom-[100%] right-4 left-4 mb-2 z-50 shadow-2xl rounded-3xl overflow-hidden border border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 animate-in slide-in-from-bottom-2">
              <EmojiPicker 
          ...
          {replyingTo && (
            <div className="bg-white/95 dark:bg-gray-900/95 backdrop-blur-md border-r-4 border-[#2D5AF0] p-2.5 rounded-2xl mb-2 flex justify-between items-center shadow-lg border border-gray-100 dark:border-gray-800 animate-in fade-in slide-in-from-bottom-2">
              <div className="flex flex-col overflow-hidden pl-2">
                ...
                <span className="text-xs text-brand-dark/70 dark:text-gray-300 truncate">{replyingTo.content || (replyingTo.media_url ? 'מדיה' : '')}</span>
              </div>
          ...
          {editingMsgId && (
            <div className="text-xs text-[#2D5AF0] font-bold mb-2 flex justify-between items-center bg-white/95 dark:bg-gray-900/95 backdrop-blur-md rounded-2xl p-2.5 border border-gray-100 dark:border-gray-800 shadow-lg animate-in fade-in">
          ...
          <form onSubmit={handleSend} className="flex items-center gap-1 bg-white dark:bg-gray-800 p-1 pr-2 rounded-full border border-gray-200 dark:border-gray-700 shadow-xl pointer-events-auto">
            ...
            <input type="text" value={newMessage} onChange={(e) => setNewMessage(e.target.value)} onFocus={() => setShowEmoji(false)} placeholder={editingMsgId ? "ערוך הודעה..." : "הודעה..."} className="flex-1 bg-transparent py-2 px-1 outline-none text-sm text-brand-dark dark:text-white" />
