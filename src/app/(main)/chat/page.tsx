...
export default function ChatPage() {
  ...
  const triggerAiAnalysis = async (msgs: any[]) => {
    if (msgs.length === 0) {
      setAiSummary("הצ'אט שקט כרגע, היה הראשון לכתוב! ✨")
    ...
    try {
      const res = await fetch('/api/ai/analyze', {
        ...
        body: JSON.stringify({
          description: `אלו ההודעות האחרונות בצ'אט הבניין:\n${contextMsgs}\n\nנסח משפט סיכום קצר ואלגנטי של שורה אחת שאומר על מה מדברים עכשיו. מקסימום אימוג'י אחד.`,
          mode: 'insight'
        })
      })
      if (res.ok) {
        ...
        setAiSummary("מדברים על ענייני השעה בבניין ✨")
      }
    } catch (e) {
    ...
    const initChat = async () => {
      ...
      const fetchMessages = async () => {
        ...
        if (data && isMounted) {
          ...
          if (lastSummaryCountRef.current !== data.length) {
            lastSummaryCountRef.current = data.length
  ...
  const timeFormat = (dateString: string) => new Date(dateString).toLocaleTimeString('he-IL', { hour: '2-digit', minute: '2-digit' })
    ...
    <div className="flex flex-col flex-1 w-full relative min-h-[100dvh]" dir="rtl">
        ...
        {messages.map((msg) => {
          ...
          return (
            <div key={msg.id} className={`flex gap-2 relative ${isMe ? 'flex-row-reverse' : ''} ${isActive ? 'z-[60]' : 'z-10'}`}>
              {!isMe && (
                <img 
                  src={msg.profiles?.avatar_url || `https://api.dicebear.com/8.x/initials/svg?seed=${msg.profiles?.full_name}`} 
                  className="w-8 h-8 rounded-full border border-white self-end shrink-0 shadow-sm object-cover" 
                  alt="avatar"
                />
              )}
                ...
                <div className={`w-full text-xs tracking-tight leading-relaxed shadow-sm relative z-0 transition-transform ${
                  ...
                  {repliedMsg && (
                    <div className={`p-2 rounded-t-[1.1rem] mb-1 text-right ${
                      ...
                      <span className={`font-black text-[10px] block mb-0.5 ${isMe ? 'text-blue-100' : 'text-[#1D4ED8]'}`}>
                  ...
                  <div className="p-2.5 pt-1">
                    {!isMe && (
                      <p className="font-bold text-[10px] text-[#1D4ED8] mb-1 px-0.5 text-right">
                    ...
                    <div className="flex items-center justify-end gap-1 mt-1" dir="ltr">
                      {isMe && (
                        <svg className={`w-3.5 h-3.5 ${hasBeenRead ? 'text-[#38BDF8] drop-shadow-sm' : 'text-blue-200'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                          <path d="M18 6L7 17l-5-5"></path>
                          {hasBeenRead && <path d="M22 10l-7.5 7.5L13 16"></path>}
                        </svg>
                      )}
                      <span className={`text-[9px] font-bold ${isMe ? 'text-blue-100' : 'text-slate-400'}`}>{timeFormat(msg.created_at)}</span>
                    </div>
                  </div>
                </div>
              </div>
      ...
      {editingMessage && (
        <div className="fixed inset-0 z-[110] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-[1.5rem] p-6 shadow-2xl animate-in zoom-in-95 text-right">
            ...
            <textarea autoFocus value={editContent} onChange={e => setEditContent(e.target.value)} className="w-full bg-slate-50 rounded-2xl p-4 text-xs font-medium outline-none resize-none min-h-[100px] mb-4 text-slate-800 border border-slate-100 focus:border-[#1D4ED8] transition tracking-tight leading-relaxed" />
            <div className="flex gap-3">
              <button onClick={() => setEditingMessage(null)} className="h-12 px-6 flex items-center justify-center bg-slate-100 text-slate-500 font-bold rounded-xl text-xs active:scale-95 transition">ביטול</button>
              <button onClick={handleSaveEdit} disabled={!editContent.trim() || editContent === editingMessage.content} className="flex-1 h-12 flex items-center justify-center bg-[#1D4ED8] text-white font-bold rounded-xl text-xs shadow-md active:scale-95 transition disabled:opacity-50">שמירה</button>
            </div>
          </div>
        </div>
      )}
      ...
      {fullScreenImage && (
        <div className="fixed inset-0 z-[150] bg-black/95 flex items-center justify-center animate-in fade-in zoom-in-95" onClick={() => setFullScreenImage(null)}>
          <button onClick={() => setFullScreenImage(null)} className="absolute top-6 right-6 w-12 h-12 flex items-center justify-center bg-white/10 hover:bg-white/20 rounded-full text-white transition active:scale-95 z-10">
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M6 18L18 6M6 6l12 12"></path></svg>
          </button>
      ...
      <div className="fixed bottom-0 left-0 w-full flex flex-col items-center z-50 pointer-events-none pb-4 pt-2">
        <div className="w-full max-w-md px-4 pointer-events-auto relative">
          ...
          <form onSubmit={handleSend} className="flex items-end gap-1 bg-white p-1.5 pr-1.5 rounded-[1.5rem] border border-slate-200 shadow-xl pointer-events-auto">
            <button type="button" onClick={() => setShowEmoji(!showEmoji)} className="w-12 h-12 flex items-center justify-center text-slate-400 hover:text-[#1D4ED8] transition shrink-0 self-end">
            ...
            <button type="button" onClick={() => fileInputRef.current?.click()} disabled={isUploading} className="w-12 h-12 flex items-center justify-center text-slate-400 hover:text-[#1D4ED8] transition shrink-0 self-end">
              {isUploading ? (
                <div className="w-5 h-5 border-2 border-slate-200 border-t-[#1D4ED8] rounded-full animate-spin"></div>
              ) : (
            ...
            <textarea
              ...
              className="flex-1 bg-transparent py-3.5 px-2 outline-none text-xs font-medium text-slate-800 resize-none max-h-32 min-h-[48px] tracking-tight leading-relaxed text-right"
              rows={1}
              onInput={(e) => {
            ...
            <button type="submit" disabled={!newMessage.trim() || isUploading} className="bg-[#1D4ED8] text-white w-12 h-12 rounded-full flex items-center justify-center shadow-md disabled:opacity-50 shrink-0 self-end ml-1 active:scale-95 transition">
