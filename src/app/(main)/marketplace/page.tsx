...
export default function MarketplacePage() {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel('marketplace_realtime_v2')
    ...
    <div className="flex flex-col flex-1 w-full pb-24 relative" dir="rtl">
      ...
      <div className="space-y-5 px-4">
        {items.length === 0 ? (
          ...
          items.map(item => (
            <div key={item.id} className="bg-white rounded-3xl shadow-sm border border-gray-50 flex flex-col relative overflow-hidden">
              
              {profile?.id === item.user_id && (
                <button onClick={() => handleDelete(item.id)} className="absolute top-4 left-4 p-2 bg-black/50 text-white backdrop-blur-md rounded-full hover:bg-red-500 transition z-10">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                </button>
              )}

              {/* תצוגת מדיה (תמונה/סרטון) עם פרופורציה מושלמת */}
              {item.media_url && (
              ...
              <div className="p-5">
                {/* אם אין תמונה, נציג את המוכר כאן */}
                {!item.media_url && (
