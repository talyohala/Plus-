...
export default function NotificationsPage() {
    ...
    useEffect(() => {
        const fetchNotifications = async () => {
            ...
            const { data } = await supabase
                ...
                .limit(50) // שומר על ביצועים, מציג 50 אחרונות
    ...
    const markAsReadAndNavigate = async (notification: any) => {
        ...
        if (!notification.is_read) {
            // מסמן כנקרא במסד הנתונים
            await supabase.from('notifications').update({ is_read: true }).eq('id', notification.id)
            // מעדכן את ה-UI מקומית כדי שירגיש מהיר
    ...
    const deleteNotification = async (e: React.MouseEvent, id: string) => {
        e.stopPropagation() // מונע מעבר לעמוד אחר בלחיצה על המחיקה
        playSystemSound('click')
    ...
    const getIconConfig = (type: string) => {
        switch (type) {
            case 'chat': return { bg: 'bg-emerald-50 text-emerald-500', icon: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"></path> }
            case 'payment': return { bg: 'bg-[#1D4ED8]/10 text-[#1D4ED8]', icon: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path> }
            case 'event': return { bg: 'bg-rose-50 text-rose-500', icon: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path> }
            case 'marketplace': return { bg: 'bg-purple-100 text-purple-600', icon: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"></path> }
            case 'system': return { bg: 'bg-orange-50 text-orange-500', icon: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path> }
            default: return { bg: 'bg-slate-100 text-slate-500', icon: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"></path> }
        }
    }
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <button 
                                        ...
                                        className="absolute left-3 top-1/2 -translate-y-1/2 p-2 bg-red-50 text-red-500 rounded-full opacity-0 group-hover:opacity-100 transition-opacity"
                                    >
