...
export default function NotificationsPage() {
  ...
  const getIconConfig = (type: string) => {
    switch (type) {
      case 'join_request': return { bg: 'bg-amber-50 text-amber-600', icon: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z"></path> }
    ...
    <div className="flex flex-col flex-1 w-full pb-32 bg-transparent min-h-[100dvh] relative" dir="rtl">
      ...
      <div className="px-6 pt-6 pb-4 flex justify-between items-center sticky top-0 z-30 bg-transparent">
        <div className="flex items-center gap-3">
          ...
          {unreadCount > 0 && (
            <span className="bg-[#1D4ED8] text-white text-[11px] font-black px-2.5 py-0.5 rounded-full shadow-sm animate-pulse">
              {unreadCount}
            </span>
          )}
        </div>
        {unreadCount > 0 && (
          <button onClick={markAllAsRead} className="text-xs font-bold text-[#1D4ED8] bg-[#1D4ED8]/5 backdrop-blur-md px-4 min-h-[48px] flex items-center justify-center rounded-full border border-[#1D4ED8]/10 active:scale-95 transition shadow-sm hover:bg-[#1D4ED8]/10">
            סמן הכל כנקרא
          </button>
        )}
      </div>

      <div className="px-5 mt-2 flex-1 relative z-10">
              ...
              return (
                <div key={notif.id} className="relative group select-none [-webkit-touch-callout:none]">
                  ...
                  <div
                    ...
                    className={`relative p-4 rounded-[1.5rem] border shadow-sm transition-all active:scale-[0.98] cursor-pointer flex gap-4 overflow-hidden ${
                      notif.is_read ? 'bg-white/60 border-white/50 opacity-80' : 'bg-white border-[#1D4ED8]/15 shadow-[0_4px_15px_rgba(29,78,216,0.06)]'
                    }`}
                  >
                    ...
                    <div className="flex-1 min-w-0 pr-1 pointer-events-none text-right">
                      <div className="flex justify-between items-start mb-0.5">
                        <h4 className={`text-xs tracking-tight pr-2 truncate ${notif.is_read ? 'font-bold text-slate-700' : 'font-black text-slate-900'}`}>{notif.title}</h4>
            ...
            <div className="flex flex-col gap-3">
              <button onClick={() => deleteNotification(activeActionMenu.id)} className="w-full flex items-center justify-between bg-white border border-red-100 p-4 rounded-xl hover:border-red-200 transition active:scale-95 shadow-sm group">
                <div className="flex items-center gap-3 w-full flex-row-reverse">
                  <div className="w-12 h-12 rounded-full bg-red-50 text-red-500 flex items-center justify-center group-hover:bg-red-500 group-hover:text-white transition shrink-0">
