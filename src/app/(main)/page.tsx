...
export default function HomePage() {
  ...
  const [aiCatchup, setAiCatchup] = useState<string>('טוען נתונים בזמן אמת...');
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (prof) {
        ...
        const [payRes, tickRes, msgRes, marketRes, eventsRes] = await Promise.all([
        ...
        if (marketRes.data) setMarketItems(marketRes.data);

        const currentHash = `${activeTickets}-${myUnpaid}-${marketRes.data?.length || 0}-${msgRes.data?.[0]?.content || ''}`;
        ...
        if (lastDataHashRef.current !== currentHash) {
            ...
            const context = `
                דייר: ${prof.full_name}.
                בזמן אמת בבניין: ${activeTickets} תקלות פתוחות, ${myUnpaid} תשלומים ממתינים. בלוח: ${marketRes.data?.length || 0} פריטים.
                הודעה אחרונה בצ'אט: "${msgRes.data?.[0]?.content || 'אין'}".
                נסח עדכון Catch-up ריל-טיים קצרצר של 2 שורות מגוף ראשון כדובר המערכת. התמקד במה שקורה כרגע. אימוג'י 1 בלבד.
            `;
            ...
            try {
                ...
                setAiCatchup(`עדכון חם: ${activeTickets} תקלות בטיפול ו-${myUnpaid} תשלומים ממתינים. יום נפלא!`);
            }
        }
      }
    } catch (err) {
  ...
  const handleParkingRequest = async () => {
      ...
      try {
          const res = await fetch('/api/ai/analyze', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ description: `נסח הודעה קצרה לקבוצת הבניין: אני מחפש חניה פנויה להיום, מי יכול להשאיל? אימוג'י אחד בלבד.`, mode: 'insight' })
          });
          ...
          setCustomAlert({ title: 'נשלח לקהילה', message: 'בקשת החניה נוסחה ונשלחה בהצלחה לצ\'אט הבניין.', type: 'success' });
          playSystemSound('notification');
      } finally {
    ...
    <div className="flex flex-col flex-1 w-full min-h-[100dvh] bg-transparent pb-40 relative" dir="rtl">
          ...
          <button onClick={handleGateRemote} className="flex flex-col items-center gap-2 group active:scale-95 transition-transform">
              <div className="w-16 h-16 bg-white rounded-full shadow-[0_8px_20px_rgba(29,78,216,0.1)] border border-blue-50 flex items-center justify-center text-[#1D4ED8] group-hover:bg-blue-50 transition-colors">
                  <svg className="w-7 h-7" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
                      ...
                      <path d="M12 16v2" />
                      <path d="M5 6a8 8 0 0 0 0 6" />
                      <path d="M19 6a8 8 0 0 1 0 6" />
                  </svg>
              </div>
      ...
      <div className="px-6 relative z-10 grid grid-cols-2 gap-4 mb-8">
        <Link href="/payments" onClick={() => playSystemSound('click')} className={`bg-white rounded-[1.5rem] p-4 shadow-[0_4px_15px_rgba(29,78,216,0.05)] border border-[#1D4ED8]/20 flex flex-col items-center justify-center text-center active:scale-95 transition-transform`}>
            <div className="w-12 h-12 rounded-full flex items-center justify-center mb-2 bg-blue-50 text-[#1D4ED8]">
            ...
            <span className={`text-base font-black ${unpaidCount && unpaidCount > 0 ? 'text-[#1D4ED8]' : 'text-slate-800'}`}>
        ...
        <Link href="/services" onClick={() => playSystemSound('click')} className={`bg-white rounded-[1.5rem] p-4 shadow-[0_4px_15px_rgba(29,78,216,0.05)] border border-[#1D4ED8]/20 flex flex-col items-center justify-center text-center active:scale-95 transition-transform`}>
            <div className="w-12 h-12 rounded-full flex items-center justify-center mb-2 bg-orange-50 text-orange-500">
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77-1.333.192 3 1.732 3z" /></svg>
            </div>
            <span className="text-xs font-bold text-slate-500">תקלות שירות</span>
            <span className={`text-base font-black ${openTickets && openTickets > 0 ? 'text-[#1D4ED8]' : 'text-slate-800'}`}>
        ...
        <Link href="/events" onClick={() => playSystemSound('click')} className={`bg-white rounded-[1.5rem] p-4 shadow-[0_4px_15px_rgba(29,78,216,0.05)] border border-[#1D4ED8]/20 flex flex-col items-center justify-center text-center active:scale-95 transition-transform`}>
            <div className="w-12 h-12 rounded-full flex items-center justify-center mb-2 bg-rose-50 text-rose-500">
            ...
            <span className={`text-base font-black ${upcomingEventsCount && upcomingEventsCount > 0 ? 'text-[#1D4ED8]' : 'text-slate-800'}`}>
                {upcomingEventsCount === null ? '-' : upcomingEventsCount > 0 ? `${upcomingEventsCount} קרובים` : 'אין אירועים'}
            </span>
        </Link>
      ...
      <div className="px-6 mb-8 relative z-10">
        <div className="flex justify-between items-center mb-3">
          ...
          <Link href="/marketplace" className="text-xs font-bold text-[#1D4ED8] hover:underline">
            לכל העדכונים ({marketItems.length})
          </Link>
        </div>
      ...
      <div className="fixed bottom-[84px] left-0 right-0 max-w-md mx-auto w-full px-4 py-2 z-40 bg-transparent pointer-events-none">
          <div className="pointer-events-auto">
              <form onSubmit={handleMagicSubmit} className="relative bg-white rounded-[2rem] p-2 shadow-[0_8px_30px_rgba(29,78,216,0.15)] border border-[#1D4ED8]/20 flex items-end gap-2 transition-all">
                  <button type="button" onClick={() => setCustomAlert({title: 'העלאת תמונות', message: 'בקרוב ה-AI יוכל לנתח גם תמונות!', type: 'info'})} className="w-10 h-10 flex items-center justify-center text-slate-400 hover:text-[#1D4ED8] bg-slate-50 rounded-full transition shrink-0 self-end mb-0.5">
                  ...
                  <textarea 
                    ...
                    placeholder="מה קורה בבניין? (תקלה, חבילה...)"
                    className="flex-1 bg-transparent py-2.5 px-1 outline-none text-xs font-bold text-slate-800 resize-none max-h-32 min-h-[44px] placeholder-slate-400 self-center tracking-tight"
                    rows={1}
                    onInput={(e) => {
                  ...
                  />
                  <button type="submit" disabled={!magicInput.trim() || isMagicThinking} className="bg-[#1D4ED8] text-white w-12 h-12 rounded-full flex items-center justify-center shadow-[0_4px_15px_rgba(29,78,216,0.3)] disabled:opacity-50 shrink-0 self-end mb-0.5 active:scale-95 transition">
      ...
      {fullScreenMedia && (
        <div className="fixed inset-0 z-[150] bg-black/95 backdrop-blur-xl flex items-center justify-center p-4 animate-in fade-in cursor-pointer" onClick={() => setFullScreenMedia(null)}>
          <button className="absolute top-6 right-6 w-12 h-12 flex items-center justify-center text-white bg-white/10 hover:bg-white/20 rounded-full transition z-10 border border-white/20">
            ✕
          </button>
      ...
      {customAlert && (
        <div className="fixed inset-0 z-[200] bg-black/40 backdrop-blur-sm flex justify-center items-center p-4">
          <div className="bg-white/95 backdrop-blur-xl rounded-[2rem] p-6 w-full max-w-sm shadow-2xl text-center animate-in zoom-in-95 border border-white/50">
            <div className={`w-20 h-20 rounded-full mx-auto mb-4 flex items-center justify-center shadow-lg ${customAlert.type === 'success' ? 'bg-[#1D4ED8]/10 text-[#1D4ED8] animate-[bounce_1s_infinite]' : customAlert.type === 'info' ? 'bg-blue-50 text-[#1D4ED8]' : 'bg-red-50 text-red-500'}`}>
