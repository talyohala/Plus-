    ...
    if (prof) {
      ...
      Promise.all([
        ...
        if (eventsRes && !eventsRes.error && eventsRes.data && eventsRes.data.length > 0) {
          // מציג את האירוע הראשון שהוא פעיל, או אם כולם מוקפאים אז יציג מוקפא
  ...
  return (
    <div className="flex flex-col flex-1 w-full pb-24 space-y-6 relative" dir="rtl">
      ...
      <div className="grid grid-cols-1 gap-4 px-4 relative z-10">
        ...
        {/* לוח אירועים - נעול תמיד על אדום-אפרסק */}
          ...
          <div className="flex-1 relative z-10 min-w-0">
            ...
            <p className={`text-sm font-bold ${
              ...
              {!upcomingEvent ? 'טוען נתונים...' : 
               !upcomingEvent.isPlaceholder ? (
                 upcomingEvent.status === 'frozen' ? `מוקפא: ${upcomingEvent.title} ❄️` : `בקרוב: ${upcomingEvent.title} 🎉`
               ) : 'אין אירועים קרובים 📅'}
            </p>
          </div>
