  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (prof) {
        ...
        // מנוע התאמה אישית דינמי (5 מודעות)
        ...
        // פרומפט מושחז: דורש משפט אחד בלבד, חד וקולע
  ...
  const handleParkingRequest = async () => {
    ...
    try {
      const res = await fetch('/api/ai/analyze', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ description: `כתוב בקשת חניה קצרה לקבוצת הבניין מגוף ראשון. משפט אחד. אימוג'י 1.`, mode: 'insight' })
      });
      let msg = "מישהו יכול להשאיל חניה להיום? 🙏";
      if (res.ok) {
        const data = await res.json();
        msg = data.text;
      }
      await supabase.from('messages').insert([{ user_id: profile.id, building_id: building.id, content: msg, read_by: [] }]);
      setCustomAlert({ title: 'נשלח לקהילה', message: 'בקשת החניה נשלחה לצ\'אט.', type: 'success' });
      playSystemSound('notification');
    } finally {
