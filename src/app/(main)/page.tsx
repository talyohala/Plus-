...
export default function HomePage() {
  ...
  const fetchData = async (user: any) => {
    ...
    // משיכת פוסטים - נעוצים נשארים למעלה לנצח בזכות השורה הזו
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel('feed_realtime_v2')
  ...
  // אינטראקציות
  const toggleLike = async (postId: string, isLiked: boolean, likeId?: string) => {
    if (!profile) return
    if (isLiked && likeId) {
  ...
  const handleAddComment = async (e: React.FormEvent, postId: string) => {
    ...
    await supabase.from('comments').insert([{ post_id: postId, user_id: profile.id, content: commentContent }])
    setCommentContent('')
  }

  // ניהול מנהלים (נעיצה ומחיקה)
        ...
        {posts.map(post => {
          ...
          return (
            <div key={post.id} className={`bg-white p-5 rounded-3xl shadow-sm border flex flex-col relative transition-all ${post.is_pinned ? 'border-brand-blue/30 shadow-[0_4px_20px_rgba(0,68,204,0.1)]' : 'border-gray-50'}`}>
              ...
              {/* תפריט 3 נקודות (מוצג למנהל או ליוצר הפוסט) */}
              {(isOwner || isAdmin) && (
              ...
              <div className="flex items-center gap-4 pt-3 border-t border-gray-50">
                <button onClick={() => toggleLike(post.id, isLiked, myLike?.id)} className={`flex items-center gap-1.5 text-xs font-bold transition active:scale-95 ${isLiked ? 'text-red-500' : 'text-gray-400 hover:text-red-400'}`}>
                  ...
                  {post.comments?.map((c: any) => (
                    <div key={c.id} className="flex gap-2 items-start">
                      ...
                      <div className="bg-gray-50 rounded-2xl rounded-tr-none px-3 py-2 flex-1">
                        <p className="text-[10px] font-bold text-brand-dark flex items-center gap-1">
