...
const fetcher = async () => {
  ...
  const { data: prof } = await supabase.from('profiles').select('*, buildings(*)').eq('id', userId).single();
  if (!prof) throw new Error('Profile not found');

  const today = new Date(); today.setHours(0, 0, 0, 0);

  const [payRes, tickRes, msgRes, marketRes, eventsRes, savesRes, aiReqRes] = await Promise.all([
    supabase.from('payments').select('id, status').eq('payer_id', userId).neq('status', 'paid').neq('status', 'exempt'),
    supabase.from('service_tickets').select('id').eq('building_id', prof.building_id).neq('status', 'טופל'),
    supabase.from('messages').select('content').eq('building_id', prof.building_id).order('created_at', { ascending: false }).limit(1),
    supabase.from('marketplace_items').select('*, profiles(full_name, avatar_url, apartment, floor, role)').eq('building_id', prof.building_id).eq('status', 'available').order('created_at', { ascending: false }).limit(30),
    supabase.from('events').select('id').eq('building_id', prof.building_id).gte('event_date', today.toISOString()),
    supabase.from('marketplace_saves').select('item_id').eq('user_id', prof.id),
    supabase.from('ai_smart_requests').select('*').eq('user_id', userId).gt('expires_at', new Date().toISOString()).order('created_at', { ascending: false }).limit(1)
  ]);

  return {
    profile: prof,
    building: prof.buildings,
    unpaidCount: payRes.data?.length || 0,
    openTickets: tickRes.data?.length || 0,
    upcomingEvents: eventsRes.data?.length || 0,
    lastMessage: msgRes.data?.[0]?.content || '',
    marketItems: marketRes.data || [],
    saves: new Set(savesRes.data?.map(s => s.item_id) || []),
    aiRequest: aiReqRes.data?.[0] || null
  };
};

export default function HomePage() {
  const router = useRouter();
  const { data, error, mutate } = useSWR('/api/home/fetch', fetcher, { revalidateOnFocus: true });

  const profile = data?.profile;
  const building = data?.building;
  const savesSet = data?.saves || new Set<string>();
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`home_${profile.building_id}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'payments', filter: `payer_id=eq.${profile.id}` }, () => mutate())
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'ai_smart_requests', filter: `user_id=eq.${profile.id}` }, () => mutate())
      .subscribe();
    return () => { supabase.removeChannel(channel); };
  }, [profile?.building_id, profile?.id, mutate]);

  // --- Personalized Marketplace Items (Top 5) ---
  const marketItems = useMemo(() => {
    if (!data?.marketItems) return [];
    const preferredCategories = new Set<string>();
    data.saves.forEach(savedId => {
      const found = data.marketItems.find(it => it.id === savedId);
      if (found) preferredCategories.add(found.category);
    });

    const timePeriodSeed = Math.floor(Date.now() / (1000 * 3600 * 4));
    const scoredItems = data.marketItems.map(item => {
      let score = 0;
      ...
      for (let i = 0; i < item.id.length; i++) { hash = ((hash << 5) - hash) + item.id.charCodeAt(i); hash |= 0; }
      score += Math.abs(hash) % 40;
      return { item, score };
    });

    return scoredItems.sort((a, b) => b.score - a.score).slice(0, 5).map(s => s.item);
  }, [data?.marketItems, data?.saves]);

  // --- AI Catchup Generator ---
  useEffect(() => {
    if (!data) return;
    const currentHash = `${data.openTickets}-${data.unpaidCount}-${marketItems.length}-${data.lastMessage}`;
    if (lastDataHashRef.current === currentHash) return;
    lastDataHashRef.current = currentHash;

    const generateInsight = async () => {
      const context = `בניין: ${profile?.full_name}. ${data.openTickets} תקלות, ${data.unpaidCount} תשלומים, ${marketItems.length} בלוח. צ'אט: "${data.lastMessage || 'אין'}". כתוב משפט סיכום קצר, קולע ומזמין באווירה שכונתית מגוף ראשון (כעוזר אישי). אימוג'י 1 בלבד.`;
      try {
        const aiRes = await fetch('/api/ai/analyze', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ description: context, mode: 'insight' }) });
        if (aiRes.ok) {
          const aiData = await aiRes.json();
          setAiCatchup(aiData.text || `הכל תקין בבניין. יום נפלא! ✨`);
        }
      } catch (e) {
    ...
    generateInsight();
  }, [data, profile, marketItems.length]);

  // --- Actions ---
  const handleShowCode = () => {
    if (!building?.entry_code) { setCustomAlert({ title: 'אין קוד כניסה', message: 'לא הוגדר קוד דלת לבניין זה.', type: 'info' }); return; }
    playSystemSound('click'); setShowCode(true); setTimeout(() => setShowCode(false), 4000);
  };

  const handleGateRemote = () => {
    playSystemSound('click');
    setCustomAlert({ title: 'פותח שער...', message: 'מתחבר לשער החניה.', type: 'info' });
    setTimeout(() => { playSystemSound('notification'); setCustomAlert({ title: 'השער נפתח', message: 'נסיעה בטוחה!', type: 'success' }); }, 1500);
  };

  const handleParkingRequest = async () => {
    if (!profile || !building) return;
    setIsAskingParking(true); playSystemSound('click');
    try {
      const { data: req, error } = await supabase.from('ai_smart_requests').insert({ building_id: building.id, user_id: profile.id, type: 'parking', status: 'searching' }).select().single();
      if (error) throw new Error(error.message);
      const msg = `🚗 היי שכנים, אני מחפש/ת חניה ל-24 השעות הקרובות.\nלמישהו יש משהו פנוי להציע?\n\n[AI_REQ:${req.id}]`;
      await supabase.from('messages').insert([{ user_id: profile.id, building_id: building.id, content: msg, read_by: [] }]);
      playSystemSound('notification'); mutate();
    } catch (err: any) {
      setCustomAlert({ title: 'שגיאה', message: err.message || 'אירעה שגיאה בבקשת החניה', type: 'error' });
    } finally { setIsAskingParking(false); }
  };

  const handleAckParking = async () => {
    if (!data?.aiRequest) return;
    playSystemSound('click');
    setCustomAlert({ title: 'יש לך חניה! 🎉', message: `${data.aiRequest.matched_name} דואג לך. כנס/י לצ'אט קבוצת הבניין לתאם מולו!`, type: 'success' });
    await supabase.from('ai_smart_requests').delete().eq('id', data.aiRequest.id);
    mutate();
  };
  ...
  const handleMagicSubmit = async (e: React.FormEvent) => {
    ...
    playSystemSound('click'); setIsMagicThinking(true);
    try {
      const res = await fetch('/api/ai/omni', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text, userId: profile.id, buildingId: building.id }) });
      const resData = await res.json();
      if (res.ok) {
        playSystemSound('notification'); setMagicInput('');
        if (inputRef.current) inputRef.current.style.height = 'auto';
        const actionTitle = resData.action === 'TICKET' ? 'תקלה נפתחה 🛠️' : resData.action === 'MARKETPLACE' ? 'פורסם בלוח 🤝' : 'נשלח לצ\'אט 💬';
        setCustomAlert({ title: actionTitle, message: 'הפקודה עובדה בהצלחה ע"י ה-AI.', type: 'success' });
        mutate();
      } else throw new Error(resData.error || 'שגיאת עיבוד');
    } catch (err: any) {
      setCustomAlert({ title: 'שגיאה', message: 'לא הצלחנו לנתב את הפנייה.', type: 'error' });
    } finally { setIsMagicThinking(false); }
  };

  const toggleSave = async (e: React.MouseEvent, id: string, isCurrentlySaved: boolean) => {
    e.stopPropagation(); setOpenMenuId(null); if (!profile) return; playSystemSound('click');
    if (isCurrentlySaved) await supabase.from('marketplace_saves').delete().match({ item_id: id, user_id: profile.id });
    else await supabase.from('marketplace_saves').insert([{ item_id: id, user_id: profile.id }]);
    mutate();
  };

  const togglePin = async (id: string, currentStatus: boolean) => {
    await supabase.from('marketplace_items').update({ is_pinned: !currentStatus }).eq('id', id);
    setOpenMenuId(null); playSystemSound('click'); mutate();
  };
  ...
  const handleInlineEditSubmit = async (e: React.FormEvent, id: string) => {
    e.preventDefault();
    const parsedPrice = ['למסירה', 'חבילות ודואר', 'השאלות כלים'].includes(editItemData.category) ? 0 : parseFloat(editItemData.price) || 0;
    ...
    playSystemSound('notification'); setEditingItemId(null); mutate();
  };
  ...
  return (
    <div className="flex flex-col flex-1 w-full min-h-[100dvh] bg-transparent pb-40 relative" dir="rtl" onClick={() => setOpenMenuId(null)}>
      {mounted && successModal && createPortal(
        <div className="fixed inset-0 z-[99999] bg-black/40 backdrop-blur-sm flex justify-center items-center p-4" onClick={() => setSuccessModal(false)} dir="rtl">
          <div className="bg-white rounded-[2rem] p-6 w-full max-w-sm shadow-2xl text-center border border-white/50 animate-in zoom-in-95" onClick={e => e.stopPropagation()}>
            <div className="w-20 h-20 rounded-full mx-auto mb-4 flex items-center justify-center bg-[#059669]/10 text-[#059669] shadow-sm animate-[bounce_1s_infinite]"><svg className="w-10 h-10" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="3"><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg></div>
        ...
        <div className="fixed inset-0 z-[99999] bg-black/40 backdrop-blur-sm flex justify-center items-center p-4" onClick={() => setCustomAlert(null)} dir="rtl">
          <div className="bg-white rounded-[2rem] p-6 w-full max-w-sm shadow-2xl text-center border border-white/50 animate-in zoom-in-95" onClick={e => e.stopPropagation()}>
            <div className={`w-20 h-20 rounded-full mx-auto mb-4 flex items-center justify-center shadow-sm ${customAlert.type === 'success' ? 'bg-emerald-50 text-emerald-600 animate-[bounce_1s_infinite]' : customAlert.type === 'info' ? 'bg-blue-50 text-[#1D4ED8]' : 'bg-red-50 text-red-500'}`}>
              {customAlert.type === 'success' ? <svg className="w-10 h-10" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="3"><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg> : <svg className="w-10 h-10" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2.5"><path strokeLinecap="round" strokeLinejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>}
            </div>
            <h3 className="text-2xl font-black text-slate-800 mb-2">{customAlert.title}</h3><p className="text-base text-slate-500 mb-6 leading-relaxed font-medium">{customAlert.message}</p>
      ...
      {mounted && fullScreenMedia && createPortal(
        <div className="fixed inset-0 z-[99999] bg-black/95 backdrop-blur-xl flex items-center justify-center p-4 animate-in fade-in cursor-pointer" onClick={() => setFullScreenMedia(null)}>
          ...
          {fullScreenMedia.type === 'video' ? <video src={fullScreenMedia.url} controls autoPlay className="max-w-full max-h-[90vh] object-contain rounded-2xl shadow-2xl" onClick={e => e.stopPropagation()} /> : <img src={fullScreenMedia.url} className="max-w-full max-h-[90vh] object-contain rounded-2xl shadow-2xl" onClick={e => e.stopPropagation()} />}
        </div>, document.body
      )}
      ...
      <div className="px-6 -mt-8 relative z-20 flex justify-center gap-6 mb-6 shrink-0">
        ...
        <button onClick={handleShowCode} className="flex flex-col items-center gap-2 group active:scale-95 transition-transform min-w-[64px]">
          <div className="w-16 h-16 bg-white rounded-full shadow-[0_8px_20px_rgba(29,78,216,0.1)] border border-blue-50 flex items-center justify-center text-[#1D4ED8] group-hover:bg-blue-50 transition-colors">
            {showCode ? <span className="font-black text-xs tracking-widest text-[#1D4ED8] animate-in zoom-in-95" dir="ltr">{building?.entry_code}</span> : <svg className="w-7 h-7 animate-in zoom-in-95" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z"></path></svg>}
          </div>
          <span className="text-[11px] font-black text-slate-600 tracking-tight">קוד כניסה</span>
        </button>

        {data?.aiRequest?.status === 'matched' ? (
          <button onClick={handleAckParking} className="flex flex-col items-center gap-2 group active:scale-95 transition-transform">
            <div className="w-16 h-16 bg-emerald-500 rounded-full shadow-[0_8px_20px_rgba(16,185,129,0.3)] border border-emerald-400 flex items-center justify-center text-white transition-colors relative"><span className="text-2xl animate-[bounce_2s_infinite]">🎉</span></div>
          ...
          <button className="flex flex-col items-center gap-2 group transition-transform opacity-90 cursor-default">
            <div className="w-16 h-16 bg-[#1D4ED8] rounded-full shadow-[0_8px_20px_rgba(29,78,216,0.3)] border border-blue-400 flex items-center justify-center text-white transition-colors animate-pulse"><span className="text-[10px] font-black text-center px-1 leading-tight">מפעיל<br/>בינה...</span></div>
            <span className="text-[11px] font-black text-[#1D4ED8] tracking-tight">בקשת חניה</span>
          </button>
        ) : data?.aiRequest?.status === 'searching' ? (
          <button className="flex flex-col items-center gap-2 group transition-transform opacity-90 cursor-default">
            <div className="w-16 h-16 bg-[#1D4ED8]/10 rounded-full shadow-inner border border-[#1D4ED8]/20 flex items-center justify-center text-[#1D4ED8] transition-colors relative animate-pulse"><div className="w-6 h-6 border-2 border-[#1D4ED8] border-t-transparent rounded-full animate-spin"></div></div>
          ...
          <button onClick={handleParkingRequest} className="flex flex-col items-center gap-2 group active:scale-95 transition-transform">
            <div className="w-16 h-16 bg-white rounded-full shadow-[0_8px_20px_rgba(29,78,216,0.1)] border border-blue-50 flex items-center justify-center text-[#1D4ED8] group-hover:bg-blue-50 transition-colors"><svg className="w-7 h-7" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 0 0 3-3V8a3 3 0 0 0-3-3H6a3 3 0 0 0-3 3v8a3 3 0 0 0 3 3z" /><path d="M4 10l1.5-4h13l1.5 4" /></svg></div>
      ...
      <div className="px-6 relative z-10 grid grid-cols-2 gap-4 mb-8">
        <Link href="/payments" onClick={() => playSystemSound('click')} className="bg-white rounded-[1.5rem] p-4 shadow-[0_4px_15px_rgba(29,78,216,0.05)] border border-[#1D4ED8]/20 flex flex-col items-center justify-center text-center active:scale-95 transition-transform">
          ...
          <span className={`text-base font-black ${data?.unpaidCount > 0 ? 'text-[#1D4ED8]' : 'text-slate-800'}`}>{data?.unpaidCount > 0 ? `${data.unpaidCount} ממתינים` : 'הכל משולם'}</span>
        </Link>
        <Link href="/services" onClick={() => playSystemSound('click')} className="bg-white rounded-[1.5rem] p-4 shadow-[0_4px_15px_rgba(29,78,216,0.05)] border border-[#1D4ED8]/20 flex flex-col items-center justify-center text-center active:scale-95 transition-transform">
          ...
          <span className={`text-base font-black ${data?.openTickets > 0 ? 'text-[#1D4ED8]' : 'text-slate-800'}`}>{data?.openTickets > 0 ? `${data.openTickets} פתוחות` : 'הבניין תקין'}</span>
        </Link>
        <Link href="/events" onClick={() => playSystemSound('click')} className="bg-white rounded-[1.5rem] p-4 shadow-[0_4px_15px_rgba(29,78,216,0.05)] border border-[#1D4ED8]/20 flex flex-col items-center justify-center text-center active:scale-95 transition-transform">
          ...
          <span className={`text-base font-black ${data?.upcomingEvents > 0 ? 'text-[#1D4ED8]' : 'text-slate-800'}`}>{data?.upcomingEvents > 0 ? `${data.upcomingEvents} קרובים` : 'אין אירועים'}</span>
        </Link>
      ...
      <div className="px-6 mb-8 relative z-10">
        ...
        {marketItems.length === 0 ? (
          <div className="bg-white/80 p-6 rounded-3xl border border-[#1D4ED8]/10 text-center shadow-sm"><p className="text-xs font-bold text-slate-400">אין עדכונים בלוח כרגע ✨</p></div>
        ) : (
          <div className="space-y-3">
            {marketItems.map(item => (
              <MarketplaceItemCard
                key={item.id} item={item} currentUserId={profile?.id} isAdmin={isAdmin} isSaved={savesSet.has(item.id)}
                openMenuId={openMenuId} editingItemId={editingItemId} editItemData={editItemData} mainCategories={mainCategories} isSubmitting={false}
                onToggleMenu={setOpenMenuId} onToggleSave={toggleSave} onTogglePin={togglePin}
                onStartEdit={it => { setEditingItemId(it.id); setEditItemData({ title: it.title, description: it.description || '', price: it.price === 0 ? '' : it.price.toString(), contact_phone: it.contact_phone, category: it.category }); setOpenMenuId(null); }}
                onCancelEdit={() => setEditingItemId(null)} onUpdateEditData={setEditItemData} onSubmitEdit={handleInlineEditSubmit} onDelete={handleDelete}
                onMediaClick={(url, type) => setFullScreenMedia({ url, type })}
                formatWhatsApp={formatWhatsApp} timeFormat={timeFormat}
              />
            ))}
          </div>
        )}
