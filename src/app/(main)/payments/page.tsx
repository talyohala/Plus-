...
export default function PaymentsPage() {
  ...
  const building = data?.building as Building | undefined;
  const allPayments = data?.payments as PaymentRecord[] || [];
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`payments_realtime_${profile.id}`)
  ...
  const pendingItems = useMemo(() => allPayments.filter(p => isAdmin ? p.status === 'pending' : p.status === 'pending' && p.payer_id === profile?.id), [allPayments, isAdmin, profile?.id]);
  const approvalItems = useMemo(() => allPayments.filter(p => isAdmin ? p.status === 'pending_approval' : false), [allPayments, isAdmin]);
  const paidItems = useMemo(() => allPayments.filter(p => isAdmin ? p.status === 'paid' : p.status === 'paid' && p.payer_id === profile?.id), [allPayments, isAdmin, profile?.id]);
  const exempts = useMemo(() => allPayments.filter(p => p.status === 'exempt'), [allPayments]);
  ...
  useEffect(() => {
    if (!profile || allPayments.length === 0) { setIsAiLoading(false); return; }
    const currentHash = `${profile.id}-${allPayments.length}`;
    ...
    const processAiAnalysis = async () => {
      ...
      try {
        let context = isAdmin 
          ? `מנהל הוועד: ${profile.full_name}. קופה: ₪${totalCollected}. פתוח לגבייה: ₪${totalPendingVal} (${pendingItems.length} דרישות). נסח ניתוח חכם, סמכותי ומכובד מגוף ראשון כרובוט העוזר. המלץ לתזכר דיירים. 3 שורות ענייניות. בלי המילה חוב. אימוג'י 1 בשורה.`
          : `דייר: ${profile.full_name}. ממתינים לו ${pendingItems.length} תשלומים. נסח תזכורת חמודה מגוף ראשון להסדיר. 3 שורות. בלי המילה חוב. אימוג'י 1 בשורה.`;
        ...
        setAiInsight(isAdmin ? `שלום ${profile.full_name}, תמונת מצב חכמה 💼\nנאספו ${totalCollected.toLocaleString()} ₪ בקופה (${pendingItems.length} דרישות) 📊\nמומלץ לשלוח תזכורת לדיירים ✨` : `היי ${profile.full_name}! תזכורת ידידותית 🚀\nממתינים להסדרה ${pendingItems.length} תשלומים 💎\nנודה להסדרתך בהקדם למען הקהילה ✨`);
      } finally {
        setIsAiLoading(false); setShowAiBubble(true); setTimeout(() => setShowAiBubble(false), 20000);
      }
    };
    processAiAnalysis();
  }, [profile, allPayments.length, isAdmin, pendingItems, totalCollected, totalPendingVal]);
    ...
    if (tenants && tenants.length > 0) {
      ...
      if (!error) {
        ...
        if (tenantsToNotify.length > 0) {
          const notifs = tenantsToNotify.map(t => ({ receiver_id: t.id, sender_id: profile.id, type: 'system', title: 'דרישת תשלום חדשה 💸', content: `ועד הבית פרסם דרישת תשלום עבור: ${newTitle}.`, link: '/payments' }));
  ...
  const deletePayment = (paymentId: string) => {
    setCustomConfirm({ title: 'מחיקה', message: 'האם למחוק תשלום זה?', onConfirm: async () => { await supabase.from('payments').delete().eq('id', paymentId); mutate(); setCustomConfirm(null); playSystemSound('click'); } });
  };

  const markAsExempt = (paymentId: string) => {
    setCustomConfirm({ title: 'הענקת פטור', message: 'הדייר יקבל פטור מתשלום זה. לאשר?', onConfirm: async () => { await supabase.from('payments').update({ status: 'exempt' }).eq('id', paymentId); mutate(); setCustomConfirm(null); playSystemSound('notification'); } });
  };
  ...
  const handleApprovePayment = async (paymentId: string, payerId: string, paymentTitle: string) => {
    ...
    if (payerId !== profile.id) await supabase.from('notifications').insert([{ receiver_id: payerId, sender_id: profile.id, type: 'system', title: 'התשלום אושר! 🎉', content: `ועד הבית אישר את התשלום: ${paymentTitle}.`, link: '/payments' }]);
    playSystemSound('notification'); setCustomAlert({ title: 'עסקה אושרה', message: 'התשלום נקלט בהצלחה.', type: 'success' }); mutate();
  };
  ...
  const processCreditCard = (e: React.FormEvent) => {
    ...
    setTimeout(async () => {
      ...
      playSystemSound('notification'); setPayingItem(null); mutate();
      setCustomAlert({ title: 'עסקה אושרה!', message: 'התשלום באשראי עבר בהצלחה.', type: 'success' });
    }, 2000);
  };
  ...
  const renderList = (list: PaymentRecord[], type: 'pending' | 'approval' | 'history') => {
    ...
    const sortedList = [...list].sort((a, b) => (a.is_pinned === b.is_pinned ? 0 : a.is_pinned ? -1 : 1));
            ...
            <div key={p.id} className={`bg-white/90 backdrop-blur-xl rounded-[2rem] p-5 relative transition-all duration-300 ${p.is_pinned ? 'border-orange-200/60 bg-gradient-to-br from-orange-50/80 to-white shadow-[0_8px_25px_rgba(249,115,22,0.15)]' : 'border border-[#1D4ED8]/10 shadow-[0_8px_30px_rgba(29,78,216,0.04)]'} ${openMenuId === p.id ? 'z-50' : 'z-10'}`}>
              ...
              <div className="absolute top-0 right-0 flex overflow-hidden rounded-bl-[1.5rem] rounded-tr-[2rem] shadow-sm z-10 border-b border-l border-white/20">
                {p.is_pinned ? (
                  ...
                  <>
                    ...
                    {type === 'pending' && isOverdue && <div className="px-3 py-1.5 bg-rose-50 text-rose-600 text-[10px] font-black border-r border-rose-100/50 animate-pulse">באיחור</div>}
                  </>
                )}
              </div>
              ...
              {(isAdmin || (isPayerMe && type === 'history')) && (
                <div className="absolute top-3 left-3 z-20">
                  <button onClick={(e) => { e.stopPropagation(); setOpenMenuId(openMenuId === p.id ? null : p.id); }} className="w-8 h-8 flex items-center justify-center rounded-full text-slate-400 hover:text-[#1D4ED8] transition-colors active:scale-95"><svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path d="M10 6a2 2 0 110-4 2 2 0 010 4zM10 12a2 2 0 110-4 2 2 0 010 4zM10 18a2 2 0 110-4 2 2 0 010 4z"></path></svg></button>
                  {openMenuId === p.id && (
                    <>
                      ...
                      <div className="absolute left-0 top-10 w-[170px] bg-white/95 backdrop-blur-xl border border-slate-100 shadow-[0_15px_50px_rgba(0,0,0,0.15)] rounded-2xl z-[150] py-1.5 animate-in zoom-in-95">
                        {isAdmin && type === 'pending' && (
                          <>
                            <button onClick={() => { setEditingPaymentData({ id: p.id, title: p.title, amount: p.amount.toString() }); setOpenMenuId(null); }} className="w-full text-right px-4 h-11 text-xs font-bold text-slate-700 hover:bg-slate-50 flex items-center gap-3 border-b border-slate-100/50"><svg className="w-4 h-4 text-slate-500" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>עריכה</button>
                            <button onClick={() => { handlePersonalReminder(p); setOpenMenuId(null); }} className="w-full text-right px-4 h-11 text-xs font-bold text-slate-700 hover:bg-slate-50 flex items-center gap-3 border-b border-slate-100/50"><svg className="w-4 h-4 text-[#25D366]" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51a12.8 12.8 0 00-.57-.01c-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>שיתוף תזכורת</button>
                            <button onClick={() => { togglePinPayment(p); setOpenMenuId(null); }} className="w-full text-right px-4 h-11 text-xs font-bold text-slate-700 hover:bg-slate-50 flex items-center gap-3 border-b border-slate-100/50"><svg className="w-4 h-4 text-[#F59E0B]" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M5 5a2 2 0 012-2h10a2 2 0 012 2v16l-7-3.5L5 21V5z"></path></svg>{p.is_pinned ? 'ביטול נעיצה' : 'נעץ הודעה'}</button>
                            <button onClick={() => { markAsExempt(p.id); setOpenMenuId(null); }} className="w-full text-right px-4 h-11 text-xs font-bold text-slate-700 hover:bg-slate-50 flex items-center gap-3 border-b border-slate-100/50"><svg className="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>פטור מתשלום</button>
                            <button onClick={() => { deletePayment(p.id); setOpenMenuId(null); }} className="w-full text-right px-4 h-11 text-xs font-bold text-rose-600 hover:bg-rose-50 flex items-center gap-3"><svg className="w-4 h-4 text-rose-500" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>מחיקת דרישה</button>
                          </>
                        )}
                        {isAdmin && type === 'approval' && (
                          <>
                            <button onClick={() => { handleApprovePayment(p.id, p.payer_id, p.title); setOpenMenuId(null); }} className="w-full text-right px-4 h-11 text-xs font-bold text-emerald-600 hover:bg-emerald-50 flex items-center gap-3 border-b border-slate-100/50"><svg className="w-4 h-4 text-emerald-500" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7"></path></svg>אישור קבלה</button>
                            <button onClick={() => { deletePayment(p.id); setOpenMenuId(null); }} className="w-full text-right px-4 h-11 text-xs font-bold text-rose-600 hover:bg-rose-50 flex items-center gap-3"><svg className="w-4 h-4 text-rose-500" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12"></path></svg>דחייה ומחיקה</button>
                          </>
                        )}
                        {type === 'history' && (
              ...
              <div className="pt-7 pr-1 pl-10 select-none" onClick={() => { if (isPayerMe && type === 'pending') { setPayingItem(p); setPaymentFlowStep('select'); } }}>
                ...
                <div className="flex items-center gap-2.5 mb-3">
                  ...
                  <div className="flex flex-col">
                    <span className="text-[14px] font-black text-slate-700 leading-none flex items-center gap-1">{p.profiles?.full_name || 'דייר'}</span>
              ...
              <div className="bg-[#1D4ED8]/5 p-3.5 rounded-2xl border border-[#1D4ED8]/10 mt-2 shadow-sm flex items-center justify-between">
                <div className="flex flex-col">
                  ...
                  <div className={`text-lg font-black flex items-center justify-start gap-1 font-sans ${type === 'history' ? 'text-emerald-600' : 'text-[#1D4ED8]'}`} dir="ltr">
  ...
  return (
    <div className="flex flex-col flex-1 w-full pb-32 relative bg-transparent min-h-[100dvh]" dir="rtl" onClick={() => { setOpenMenuId(null); }}>
      ...
      {mounted && customAlert && createPortal(
        <div className="fixed inset-0 z-[10000] bg-black/40 backdrop-blur-sm flex justify-center items-center p-4" onClick={() => setCustomAlert(null)} dir="rtl">
          <div className="bg-white/95 backdrop-blur-xl rounded-[1.5rem] p-6 w-full max-w-sm shadow-2xl text-center animate-in zoom-in-95" onClick={e => e.stopPropagation()}>
            <div className={`w-20 h-20 rounded-full mx-auto mb-4 flex items-center justify-center shadow-lg ${customAlert.type === 'success' ? 'bg-[#10B981]/10 text-[#10B981]' : 'bg-blue-50 text-[#1D4ED8]'}`}>
        ...
        <div className="fixed inset-0 z-[10000] bg-black/40 backdrop-blur-sm flex justify-center items-center p-4" dir="rtl">
          <div className="bg-white/95 backdrop-blur-xl rounded-[1.5rem] p-6 w-full max-w-sm shadow-2xl text-center animate-in zoom-in-95">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={customConfirm.onConfirm} className="flex-1 h-14 bg-[#1D4ED8] text-white font-bold rounded-xl transition shadow-sm active:scale-95 text-lg">אישור</button>
            </div>
          </div>
        </div>, document.body
      )}

      <div className="px-6 pt-6 pb-4"><h2 className="text-2xl font-black text-slate-800 tracking-tight">תשלומים לוועד</h2></div>
        ...
        <div className="bg-[#1D4ED8] p-6 pt-8 rounded-[2rem] text-white shadow-xl relative overflow-hidden border border-white/10">
          ...
          <div className="relative z-10 flex flex-col items-end w-full">
            ...
            <div className="flex items-baseline justify-end gap-1.5 w-full" dir="ltr">
              <span className="text-2xl font-bold text-white">₪</span><span className="text-4xl font-black tracking-tight text-white">{totalCollected.toLocaleString()}</span>
            </div>
          </div>
          <div className="flex justify-between items-end border-t border-white/10 pt-4 relative z-10 w-full mt-1">
            <div className="flex flex-col w-1/2">
              ...
              <div className="flex items-baseline justify-end gap-1 w-full" dir="ltr"><span className="text-xs font-bold text-white/90">₪</span><span className="text-sm font-bold text-white">{totalTarget.toLocaleString()}</span></div>
            </div>
            <div className="flex flex-col w-1/2">
              <p className="text-[10px] text-white/70 font-bold mb-0.5 w-full text-left">פתוח לתשלום</p>
              <div className="flex items-baseline justify-start gap-1 w-full" dir="ltr"><span className="text-xs font-bold text-[#F5A623]">₪</span><span className="text-sm font-black text-[#F5A623]">{totalPendingVal.toLocaleString()}</span></div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="px-6 mb-5">
        <div className="flex gap-2 overflow-x-auto hide-scrollbar pb-1">
          {(isAdmin ? ['pending', 'approval', 'history'] : ['pending', 'history']).map(tabKey => {
            ...
            return (
              <button key={tabKey} onClick={() => { playSystemSound('click'); setActiveTab(tabKey as any); }} className={`px-5 h-10 rounded-full text-[13px] transition-all flex items-center justify-center font-bold gap-1.5 whitespace-nowrap shrink-0 border shadow-sm ${isActive ? 'bg-[#1D4ED8] text-white border-[#1D4ED8]' : 'bg-white text-slate-500 border-slate-200 hover:bg-slate-50'}`}>
                <span>{label}</span>
                <span className={`px-2 py-0.5 rounded-full text-[10px] font-black ${isActive ? 'bg-white/20 text-white' : 'bg-slate-100 text-slate-500'}`}>{count}</span>
              </button>
            );
          })}
        </div>
      ...
      {isAdmin && (
        <button onClick={(e) => { e.stopPropagation(); setIsCreating(true); }} className="fixed bottom-24 left-6 z-40 bg-white/90 backdrop-blur-md border border-[#1D4ED8]/20 text-slate-800 pl-4 pr-1.5 py-1.5 rounded-full shadow-[0_8px_30px_rgba(29,78,216,0.15)] hover:scale-105 active:scale-95 transition flex items-center gap-2 group flex-row-reverse">
        ...
        <form onSubmit={handleCreatePayment} className="flex flex-col relative min-h-[250px]">
          <div className="flex-1 overflow-y-auto hide-scrollbar pb-24 pt-1">
            <div className="w-full bg-[#F8FAFC] border border-slate-200 rounded-[1.5rem] p-5 focus-within:border-[#1D4ED8] focus-within:shadow-[0_0_0_4px_rgba(29,78,216,0.1)] transition-all shadow-inner relative flex flex-col gap-4">
              <input type="text" required value={newTitle} onChange={e => setNewTitle(e.target.value)} placeholder="עבור מה? (או בחר למעלה)" className="w-full bg-transparent text-xl font-black text-slate-800 placeholder-slate-400 outline-none tracking-tight" />
              <div className="h-[1px] w-full bg-slate-200/60 rounded-full"></div>
              <div className="flex flex-col">
                <span className="text-[10px] font-black text-[#1D4ED8] uppercase tracking-wider mb-1">סכום פר דייר (₪)</span>
                <input type="number" required value={newAmount} onChange={e => setNewAmount(e.target.value)} placeholder="0" dir="ltr" className="w-full bg-transparent text-4xl font-black text-[#1D4ED8] text-right outline-none placeholder-slate-300 font-mono tracking-tighter" />
              </div>
            </div>
          </div>
      ...
      <AnimatedSheet isOpen={!!editingPaymentData} onClose={() => setEditingPaymentData(null)}>
        ...
        <form onSubmit={handleInlineEditSubmit} className="flex flex-col relative min-h-[250px]">
          <div className="flex-1 overflow-y-auto hide-scrollbar pb-24 pt-1">
            <div className="w-full bg-[#F8FAFC] border border-slate-200 rounded-[1.5rem] p-5 focus-within:border-[#1D4ED8] focus-within:shadow-[0_0_0_4px_rgba(29,78,216,0.1)] transition-all shadow-inner relative flex flex-col gap-4">
              <input type="text" required value={editingPaymentData?.title || ''} onChange={e => setEditingPaymentData(prev => prev ? { ...prev, title: e.target.value } : null)} placeholder="עבור מה?" className="w-full bg-transparent text-xl font-black text-slate-800 placeholder-slate-400 outline-none tracking-tight" />
              <div className="h-[1px] w-full bg-slate-200/60 rounded-full"></div>
              <div className="flex flex-col">
                <span className="text-[10px] font-black text-[#1D4ED8] uppercase tracking-wider mb-1">סכום פר דייר (₪)</span>
                <input type="number" required value={editingPaymentData?.amount || ''} onChange={e => setEditingPaymentData(prev => prev ? { ...prev, amount: e.target.value } : null)} placeholder="0" dir="ltr" className="w-full bg-transparent text-4xl font-black text-[#1D4ED8] text-right outline-none placeholder-slate-300 font-mono tracking-tighter" />
              </div>
            </div>
          </div>
          <div className="absolute bottom-0 left-0 right-0 pt-4 bg-gradient-to-t from-white via-white to-transparent border-t border-slate-100">
      ...
      <AnimatedSheet isOpen={!!payingItem} onClose={() => { setPayingItem(null); setPaymentFlowStep('select'); }}>
        ...
        {paymentFlowStep === 'select' && (
          <div className="space-y-3 pb-8">
            <button onClick={() => selectPaymentMethod('credit')} className="w-full h-14 flex items-center justify-center bg-[#1D4ED8] text-white font-black rounded-2xl shadow-lg active:scale-95 transition-all text-lg gap-2">תשלום באשראי</button>
            <button onClick={() => selectPaymentMethod('bit')} className="w-full h-14 flex items-center justify-center bg-[#00B0FF] text-white font-black rounded-2xl shadow-lg active:scale-95 transition-all text-lg gap-2">העברה בביט</button>
            <button onClick={() => selectPaymentMethod('bank')} className="w-full h-14 flex items-center justify-center bg-[#1E293B] text-white font-black rounded-2xl shadow-lg active:scale-95 transition-all text-lg gap-2">העברה בנקאית</button>
          </div>
        )}
        {paymentFlowStep === 'credit_flow' && (
          <form onSubmit={processCreditCard} className="space-y-4 text-center pb-8">
            ...
            <div className="bg-[#1D4ED8]/5 p-4 rounded-2xl border border-[#1D4ED8]/10 mb-4 text-center"><p className="text-sm font-bold text-[#1D4ED8] mb-1">{payingItem?.title}</p><p className="text-3xl font-black text-slate-800 font-sans" dir="ltr">₪{payingItem?.amount.toLocaleString()}</p></div>
            ...
            <div className="flex gap-3"><input required type="text" placeholder="תוקף" className="w-full h-14 bg-[#F8FAFC] border border-slate-200 rounded-2xl px-4 text-sm font-black text-center outline-none focus:border-[#1D4ED8] shadow-inner" dir="ltr" /><input required type="tel" placeholder="CVV" className="w-full h-14 bg-[#F8FAFC] border border-slate-200 rounded-2xl px-4 text-sm font-black text-center outline-none focus:border-[#1D4ED8] shadow-inner" dir="ltr" /></div>
            <button type="submit" className="w-full h-14 mt-2 bg-[#1D4ED8] text-white font-black rounded-2xl shadow-lg active:scale-95 transition-all text-lg">שלם עכשיו</button>
          </form>
        )}
        {paymentFlowStep === 'bit_flow' && (
          <form onSubmit={(e) => confirmManualPayment(e, 'ביט')} className="text-center space-y-5 pb-8">
            <p className="text-sm text-slate-500 font-bold px-4 leading-relaxed">אנא אשרו כאן לאחר ביצוע ההעברה בביט:</p>
            <button type="submit" className="w-full h-14 mt-2 bg-[#00B0FF] text-white font-black rounded-2xl shadow-lg active:scale-95 transition-all text-lg">אישור, ביצעתי העברה</button>
          </form>
        )}
        {paymentFlowStep === 'bank_flow' && (
          <form onSubmit={(e) => confirmManualPayment(e, 'העברה בנקאית')} className="text-center space-y-4 pb-8">
            ...
            <div className="bg-slate-50 p-5 rounded-3xl border border-slate-200 text-right space-y-3 shadow-inner"><div className="flex justify-between items-center border-b border-slate-200 pb-2"><span className="text-slate-500 text-sm font-bold">בנק</span><span className="font-black text-slate-800">הפועלים (12)</span></div><div className="flex justify-between items-center border-b border-slate-200 pb-2"><span className="text-slate-500 text-sm font-bold">סניף</span><span className="font-black text-slate-800">123</span></div><div className="flex justify-between items-center border-b border-slate-200 pb-2"><span className="text-slate-500 text-sm font-bold">חשבון</span><span className="font-black text-slate-800">123456</span></div><div className="flex justify-between items-center"><span className="text-slate-500 text-sm font-bold">מוטב</span><span className="font-black text-slate-800">ועד הבית</span></div></div>
            <input required type="tel" placeholder="תעודת זהות (ת״ז)" value={idNumber} onChange={e => setIdNumber(e.target.value)} className="w-full h-14 bg-[#F8FAFC] border border-slate-200 rounded-2xl px-4 text-sm font-black text-right outline-none focus:border-[#1D4ED8] shadow-inner mt-4" dir="ltr" />
            <button type="submit" className="w-full h-14 mt-2 bg-[#1E293B] text-white font-black rounded-2xl shadow-lg active:scale-95 transition-all text-lg">אישור, ביצעתי העברה</button>
          </form>
        )}
        {paymentFlowStep === 'processing' && (
          <div className="text-center py-10 flex flex-col items-center justify-center gap-4 pb-12">
            <div className="w-12 h-12 border-4 border-[#1D4ED8]/20 border-t-[#1D4ED8] rounded-full animate-spin"></div>
            <div className="font-black text-xl text-[#1D4ED8] animate-pulse">מעבד תשלום...</div>
          </div>
        )}
      </AnimatedSheet>
