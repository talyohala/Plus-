...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel('profile_realtime_v4')
  ...
  // פונקציה להעלאת תמונת פרופיל חדשה
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // ניקוי המטמון של התמונה הקודמת - העלאה עם תאריך ייחודי
  ...
  return (
    <div className="flex flex-col flex-1 w-full pb-32" dir="rtl">
      ...
      <div className="px-4 mb-8">
        ...
        <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm flex flex-col gap-5 relative">
          
          <div className="flex items-center gap-4">
            
            {/* מכולת הפרופיל המעודכנת - Relative בשביל כפתור העריכה */}
            <div className="relative w-24 h-24 shrink-0">
               {/* העיגול הראשי עם object-cover מלא בלי padding */}
               <div className="w-full h-full rounded-full border-4 border-white bg-brand-blue/10 shadow-lg overflow-hidden">
                 <img 
                   ...
                   className="w-full h-full object-cover" // אובייקט-קאבר מבטיח מילוי מלא
                 />
                 
                 {/* ספינר טעינה חצי שקוף כשמעלים תמונה */}
                 {isUpdating && (
               ...
               {/* כפתור העריכה - ממוקם בפינה, בולט עם רקע לבן וצל חזק */}
               <button 
                 onClick={() => avatarInputRef.current?.click()}
                 className="absolute bottom-0 left-0 bg-white p-2 rounded-full shadow-[0_4px_15px_rgba(0,0,0,0.15)] border border-gray-100 text-brand-blue hover:scale-110 active:scale-95 transition-all z-10"
               >
                 <svg className="w-4.5 h-4.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
               </button>
            </div>
