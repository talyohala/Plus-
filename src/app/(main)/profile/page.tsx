...
export default function ProfilePage() {
  ...
  useEffect(() => {
    fetchData()
    const channel = supabase.channel('profile_realtime_v37')
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (profile.role === 'admin') {
          context = `
            ...
            נסח הודעת עזר מגוף ראשון כרובוט ניהול. תן סקירת ניהול מהירה, פנה אליו בשמו, כתוב בדיוק 3 שורות עם ירידת שורה ביניהן (\n). בלי המילה חוב. הוסף אימוג'י בכל שורה.
          `;
        } else {
          context = `
            דייר: ${profile.full_name}. גר בבניין ${building.name} שמכיל ${neighbors.length} דיירים. נותר לו לשלם סך של ${totalPendingAmount} ₪.
            נסח הודעת עזר מגוף ראשון כרובוט חמוד. היה מדויק, פנה אליו בשמו, תן סקירה אישית. כתוב בדיוק 3 שורות עם ירידת שורה ביניהן (\n). בלי המילה חוב. הוסף אימוג'י בכל שורה.
          `;
        }
    ...
    fetchAiData();
  }, [profile, building, neighbors.length, myPendingPayments.length]);
            ...
            <div className="relative flex flex-col items-end">
              {showAiBubble && (
                  <div className="absolute bottom-16 right-0 mb-2 bg-white/95 backdrop-blur-xl text-slate-800 p-4 rounded-[2rem] rounded-br-md shadow-[0_10px_40px_rgba(29,78,216,0.15)] text-[12px] font-bold w-[260px] leading-relaxed border border-white/50 animate-in fade-in slide-in-from-bottom-4 duration-500 z-50 whitespace-pre-wrap text-right pointer-events-auto">
                    {isAiLoading ? 'מעבד נתונים אישיים...' : aiInsight}
                  </div>
              )}
              <button 
                onClick={() => {
