            ...
            <div className="px-6 space-y-6 relative z-10">
                ...
                {building && !isPending && (
                    <div className={`fixed bottom-24 right-6 z-50 flex flex-col items-end pointer-events-none transition-all duration-700 ease-in-out ${isAiLoading || showAiBubble ? 'opacity-100 translate-y-0 visible' : 'opacity-0 translate-y-10 invisible'}`}>
                        ...
                        <button onClick={() => { if(showAiBubble) setShowAiBubble(false); else if(!isAiLoading) setShowAiBubble(true); }} className={`w-20 h-20 bg-transparent flex items-center justify-center pointer-events-auto active:scale-95 transition-transform duration-300 ${isAiLoading ? 'animate-pulse' : 'animate-[bounce_3s_infinite]'}`}>
                            {isAiLoading ? (
                                <div className="w-10 h-10 bg-white/50 backdrop-blur-sm rounded-full flex items-center justify-center shadow-lg border border-white"><div className="w-5 h-5 border-2 border-[#1D4ED8] border-t-transparent rounded-full animate-spin"></div></div>
                            ) : (
                    ...
                    <div className="space-y-6">
                        <div className="bg-white/60 backdrop-blur-xl border border-white/50 shadow-sm rounded-[1.5rem] p-5">
                            ...
                            <div className="flex gap-2">
                                ...
                                <button onClick={handleJoinBuilding} disabled={isUpdating || !joinBuildingCode.trim()} className="shrink-0 bg-[#1D4ED8] text-white px-6 py-4 rounded-xl text-sm font-bold active:scale-95 transition shadow-sm disabled:opacity-50">הצטרפות</button>
                            </div>
                        </div>
                        ...
                        <div className="bg-white/60 backdrop-blur-xl border border-white/50 shadow-sm rounded-[1.5rem] p-5">
                            ...
                            <div className="flex flex-col gap-3">
                                ...
                                <button onClick={handleCreateBuilding} disabled={isUpdating || !createBuildingName.trim()} className="w-full bg-slate-800 text-white py-4 rounded-xl text-sm font-bold active:scale-95 transition shadow-sm disabled:opacity-50 border border-slate-800">צור בניין חדש</button>
                            </div>
                        </div>
                    </div>
                )}
                ...
                {isPending && building && (
                    <div className="bg-white/60 backdrop-blur-xl border border-white/50 shadow-sm rounded-[1.5rem] p-5 flex flex-col gap-4">
                        <div className="bg-yellow-50/90 border border-yellow-100 rounded-2xl p-5 flex items-start gap-4 shadow-sm">
                            <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center text-yellow-500 shrink-0 shadow-sm"><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg></div>
                            <div className="pt-1">
                        ...
                        <button onClick={handleLeaveBuilding} className="w-full bg-red-50/80 text-red-500 border border-red-100 text-sm font-bold py-4 rounded-xl active:scale-95 transition flex items-center justify-center gap-2 shadow-sm">ביטול ועזיבה</button>
                    </div>
                )}
                    ...
                    <div className="space-y-6">
                            ...
                            <div className="bg-white/60 backdrop-blur-xl border border-white/50 shadow-sm rounded-[1.5rem] p-5">
                                ...
                                <div className="bg-white border border-gray-100 shadow-sm p-4 rounded-[1.5rem] flex items-center justify-between">
                                    ...
                                    <div className="flex items-center gap-2 shrink-0">
                                        <button onClick={copyBuildingCode} className="w-10 h-10 rounded-xl bg-[#2D5AF0] text-white shadow-md active:scale-95 transition flex items-center justify-center"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"></path></svg></button>
                                        <button onClick={inviteNeighbors} className="w-10 h-10 rounded-xl bg-[#25D366] text-white shadow-md active:scale-95 transition flex items-center justify-center"><svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51a12.8 12.8 0 00-.57-.01c-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg></button>
                                    </div>
                                </div>
                            </div>
                        )}
                                    ...
                                    {pendingNeighbors.map((n) => (
                                        <div key={n.id} className="flex items-center justify-between bg-white/80 border border-white shadow-sm p-3 rounded-[1.2rem]">
                                            ...
                                            <div className="flex gap-2 shrink-0">
                                                <button onClick={() => rejectNeighbor(n.id)} className="w-10 h-10 rounded-full bg-red-50/80 border border-red-100 text-red-500 flex items-center justify-center active:scale-95 transition shadow-sm"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M6 18L18 6M6 6l12 12"></path></svg></button>
                                                <button onClick={() => approveNeighbor(n.id)} className="w-10 h-10 rounded-full bg-[#25D366] border border-[#25D366]/50 text-white flex items-center justify-center active:scale-95 transition shadow-sm"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M5 13l4 4L19 7"></path></svg></button>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                        ...
                        <div>
                            ...
                            <div className="flex bg-white/60 backdrop-blur-md p-1.5 rounded-full border border-white shadow-sm relative z-10 mb-4">
                                <button onClick={() => setNeighborTab('הכל')} className={`flex-1 py-3 text-xs rounded-full transition-all flex items-center justify-center gap-1.5 ${neighborTab === 'הכל' ? 'text-[#1D4ED8] font-black bg-white shadow-sm' : 'text-slate-500 font-bold hover:text-slate-700'}`}>הכל<span className={`px-1.5 py-0.5 rounded-full text-[9px] font-black ${neighborTab === 'הכל' ? 'bg-[#1D4ED8]/10 text-[#1D4ED8]' : 'bg-gray-100 text-gray-500'}`}>{approvedNeighbors.length}</span></button>
                                <button onClick={() => setNeighborTab('הנהלה')} className={`flex-1 py-3 text-xs rounded-full transition-all flex items-center justify-center gap-1.5 ${neighborTab === 'הנהלה' ? 'text-[#1D4ED8] font-black bg-white shadow-sm' : 'text-slate-500 font-bold hover:text-slate-700'}`}>ועד<span className={`px-1.5 py-0.5 rounded-full text-[9px] font-black ${neighborTab === 'הנהלה' ? 'bg-[#1D4ED8]/10 text-[#1D4ED8]' : 'bg-gray-100 text-gray-500'}`}>{approvedNeighbors.filter(n => n.role === 'admin').length}</span></button>
                                <button onClick={() => setNeighborTab('דיירים')} className={`flex-1 py-3 text-xs rounded-full transition-all flex items-center justify-center gap-1.5 ${neighborTab === 'דיירים' ? 'text-[#1D4ED8] font-black bg-white shadow-sm' : 'text-slate-500 font-bold hover:text-slate-700'}`}>דיירים<span className={`px-1.5 py-0.5 rounded-full text-[9px] font-black ${neighborTab === 'דיירים' ? 'bg-[#1D4ED8]/10 text-[#1D4ED8]' : 'bg-gray-100 text-gray-500'}`}>{approvedNeighbors.filter(n => n.role !== 'admin').length}</span></button>
                            </div>
                                    ...
                                    displayedNeighbors.map((n) => (
                                        <div key={n.id} className="flex items-center justify-between bg-white/80 border border-white shadow-sm p-3 rounded-[1.2rem] transition hover:bg-white">
                                            ...
                                            <div className="flex gap-1.5 shrink-0 pl-1 items-center">
                                                {n.phone && (
                                                    <a href={formatWhatsAppLink(n.phone)} target="_blank" className="w-9 h-9 rounded-full bg-[#25D366] text-white shadow-sm active:scale-95 transition flex items-center justify-center border border-[#25D366]/50"><svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51a12.8 12.8 0 00-.57-.01c-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg></a>
                                                )}
                                                {isAdmin && n.id !== profile.id && (n.role !== 'admin' || isFounder) && (
                                                    <button onClick={() => toggleRole(n.id, n.role)} className={`text-[10px] font-black px-3 h-9 rounded-xl transition active:scale-95 flex items-center justify-center shadow-sm border ${n.role === 'admin' ? 'bg-red-50/80 text-red-500 border-red-100' : 'bg-white text-slate-500 border-gray-100 hover:bg-[#1D4ED8]/10 hover:text-[#1D4ED8] hover:border-[#1D4ED8]/30'}`}>{n.role === 'admin' ? 'הסר ועד' : 'מינוי'}</button>
                                                )}
                                            </div>
                                        </div>
                                    ))
                ...
                <div className="fixed inset-0 z-[100] bg-black/40 backdrop-blur-sm flex justify-center items-end" onClick={() => setIsAvatarMenuOpen(false)}>
                    <div className="bg-white/95 backdrop-blur-xl w-full max-w-md rounded-t-[2rem] p-6 pb-12 shadow-2xl animate-in slide-in-from-bottom-full border-t border-white/50" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="flex justify-between items-center mb-6 px-1">
                            ...
                            <button onClick={() => setIsAvatarMenuOpen(false)} className="w-10 h-10 bg-gray-50 border border-gray-100 rounded-xl text-slate-500 hover:text-[#1D4ED8] transition active:scale-95 flex items-center justify-center shadow-sm"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M6 18L18 6M6 6l12 12"></path></svg></button>
                        </div>
                        <div className="flex flex-col gap-4">
                            <div className="bg-white/80 p-5 rounded-[1.5rem] border border-gray-100 shadow-sm">
                                <div className="grid grid-cols-4 gap-3">
                                    {animalAvatars.map((avatar, idx) => (
                                        <button key={idx} onClick={() => updateAvatarInDB(avatar)} className="aspect-square rounded-full bg-white border border-gray-100 hover:border-[#1D4ED8] hover:shadow-md transition active:scale-90 overflow-hidden flex items-center justify-center p-2 shadow-sm"><img src={avatar} className="w-full h-full object-contain drop-shadow-sm" /></button>
                                    ))}
                                </div>
                            </div>
                            <div className="flex gap-3 mt-2">
                                <button onClick={() => avatarInputRef.current?.click()} className="flex-[2] flex items-center justify-center gap-2 bg-[#1D4ED8]/10 text-[#1D4ED8] border border-[#1D4ED8]/20 py-4 rounded-xl font-bold active:scale-95 transition shadow-sm text-sm"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>מהגלריה</button>
                                <button onClick={resetToInitials} className="flex-[1] flex items-center justify-center gap-2 bg-white shadow-sm text-slate-500 border border-gray-100 py-4 rounded-xl font-bold active:scale-95 transition text-sm hover:text-slate-800">איפוס</button>
                            </div>
                        </div>
                    </div>
                </div>
