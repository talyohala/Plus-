...
export default function ProfilePage() {
  ...
  useEffect(() => {
    fetchData()
    const channel = supabase.channel('profile_realtime_v21')
  ...
  if (isLoading) {
    return <div className="flex flex-col flex-1 w-full items-center justify-center pb-32 bg-[#F8FAFC]"><div className="w-10 h-10 border-4 border-[#E3F2FD] border-t-[#1D4ED8] rounded-full animate-spin"></div></div>
  }
  ...
  return (
    <div className="flex flex-col flex-1 w-full pb-32 bg-[#F8FAFC] min-h-screen relative" dir="rtl">
      ...
      {/* מסך עליון כהה (Cover) - מודרני, נקי, ללא אפקטים שבורים */}
      <div className="bg-slate-800 pt-12 pb-24 px-6 rounded-b-[2.5rem] shadow-md">
        <div className="flex justify-between items-center relative z-10">
          <h2 className="text-2xl font-black text-white tracking-wide">הפרופיל שלי</h2>
          <Link href="/settings" className="p-2 bg-white/10 rounded-full text-white hover:bg-white/20 transition active:scale-95">
      ...
      <div className="-mt-14 px-5 relative z-20 mb-8">
        <div className="bg-white rounded-[2rem] shadow-[0_10px_30px_rgb(0,0,0,0.05)] p-6 border border-[#E3F2FD] flex flex-col gap-6">
          ...
          <div className="flex items-center gap-5">
            ...
            <div onClick={() => setIsAvatarMenuOpen(true)} className="relative w-24 h-24 shrink-0 cursor-pointer group">
              ...
              <div className="absolute -bottom-2 -right-2 bg-white p-2 rounded-xl shadow-sm border border-slate-100 text-[#1D4ED8] group-active:scale-90 transition z-20">
          ...
          <div className="flex gap-3">
            <div className="flex-1 bg-slate-50 border border-[#E3F2FD] rounded-2xl p-3 flex flex-col">
          ...
          <button onClick={updatePersonalDetails} disabled={isUpdating} className="w-full bg-[#1D4ED8] text-white text-sm font-bold py-3.5 rounded-[1.2rem] shadow-sm active:scale-95 transition disabled:opacity-50">
            {isUpdating ? 'שומר נתונים...' : 'שמירת פרטים'}
          </button>
        </div>
      </div>

      {/* קוביית "הזמנת שכנים" פרימיום - מוצגת רק לוועד! - עיצוב נקי בלי אפקטים שבורים */}
      {building && isAdmin && (
        <div className="px-5 mb-8">
          <div className="bg-slate-800 rounded-[2rem] p-6 text-white shadow-lg">
            ...
            <div className="bg-slate-900 rounded-2xl p-4 text-center mb-4 border border-slate-700">
              <span className="text-3xl font-black tracking-[0.3em] text-white font-mono">{inviteCode}</span>
            </div>

            <button onClick={inviteNeighbors} className="w-full bg-[#25D366] text-white font-bold py-3.5 rounded-xl text-sm shadow-sm active:scale-95 transition flex items-center justify-center gap-2">
        ...
        <div className="px-5 space-y-5 mb-8">
          <div className="bg-white border border-[#E3F2FD] rounded-[2rem] p-6 shadow-sm">
            ...
            <div className="flex flex-col gap-3">
              <input type="text" value={joinBuildingCode} onChange={(e) => setJoinBuildingCode(e.target.value)} className="w-full bg-[#F8FAFC] border border-[#E3F2FD] rounded-xl px-4 py-3.5 text-sm outline-none focus:border-[#1D4ED8] text-slate-800 text-center tracking-widest font-mono font-bold uppercase" placeholder="הכנס קוד (לדוג': B-X7K9)" dir="ltr"/>
          ...
          <div className="bg-slate-800 rounded-[2rem] p-6 shadow-md text-white">
            <h3 className="text-lg font-black mb-2">הקמת בניין חדש</h3>
            <p className="text-sm text-slate-400 mb-4">אתה ועד הבית? פתח קהילה חדשה ונהל את הבניין.</p>
            <div className="flex flex-col gap-3">
              <input type="text" value={createBuildingName} onChange={(e) => setCreateBuildingName(e.target.value)} className="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3.5 text-sm outline-none focus:border-slate-500 text-white placeholder-slate-500" placeholder="שם הבניין (לדוג׳: מגדלי אלון 8)"/>
      ...
      {building && !isPending && (
        <div className="px-5 space-y-6">
          ...
          <section className="bg-white border border-[#E3F2FD] rounded-[2rem] p-5 shadow-sm">
            ...
            {isAdmin ? (
              <div className="flex flex-col gap-3">
                <input type="text" value={newBuildingName} onChange={(e) => setNewBuildingName(e.target.value)} className="w-full bg-[#F8FAFC] border border-[#E3F2FD] rounded-xl px-4 py-3.5 text-sm outline-none focus:border-[#1D4ED8] transition text-slate-800 font-bold" />
                <button onClick={updateBuildingName} disabled={isUpdating || newBuildingName === building.name} className="w-full bg-[#E3F2FD] text-[#1D4ED8] px-4 py-3.5 rounded-xl text-sm font-bold active:scale-95 transition disabled:opacity-50">
                  {isUpdating ? '...' : 'עדכן שם בניין'}
                </button>
              </div>
            ) : (
      ...
      {/* תפריט שינוי תמונת פרופיל (נשאר ללא שינוי, נקי ואלגנטי) */}
      {isAvatarMenuOpen && (
