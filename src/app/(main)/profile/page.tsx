...
// קולקציית 8 אווטארים של חיות בסגנון תלת-ממדי נקי
const animalAvatars = [
  'https://api.dicebear.com/8.x/fun-emoji/svg?seed=Cheetah&backgroundColor=eef2ff',
  'https://api.dicebear.com/8.x/fun-emoji/svg?seed=Lion&backgroundColor=eef2ff',
  'https://api.dicebear.com/8.x/fun-emoji/svg?seed=Tiger&backgroundColor=eef2ff',
  'https://api.dicebear.com/8.x/fun-emoji/svg?seed=Monkey&backgroundColor=eef2ff',
  'https://api.dicebear.com/8.x/fun-emoji/svg?seed=Elephant&backgroundColor=eef2ff',
  'https://api.dicebear.com/8.x/fun-emoji/svg?seed=Giraffe&backgroundColor=eef2ff',
  'https://api.dicebear.com/8.x/fun-emoji/svg?seed=Rhino&backgroundColor=eef2ff',
  'https://api.dicebear.com/8.x/fun-emoji/svg?seed=Zebra&backgroundColor=eef2ff'
]
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    fetchData()
    const channel = supabase.channel('profile_realtime_v9')
  ...
  return (
    <div className="flex flex-col flex-1 w-full pb-32" dir="rtl">
        ...
        <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm flex flex-col gap-5">
          ...
          <div className="flex items-center gap-4">
            ...
            <div onClick={() => setIsAvatarMenuOpen(true)} className="relative w-24 h-24 shrink-0 cursor-pointer group">
               ...
               <div className="absolute bottom-0 right-0 bg-white p-2.5 rounded-full shadow-lg border border-gray-100 text-brand-blue group-active:scale-90 transition z-20">
          ...
          {/* שדות פרטים אישיים - תיקון פרופורציות */}
      ...
      <div className="px-4 mt-12 mb-6">
        <button onClick={handleLogout} className="w-full bg-white border border-gray-100 text-gray-500 font-bold py-4 rounded-2xl hover:bg-red-50 hover:text-red-500 transition active:scale-95 shadow-sm">
          התנתק מהמערכת
        </button>
      </div>

      {/* מודל בחירת תמונת פרופיל מעוצב מחדש */}
      {isAvatarMenuOpen && (
        <div className="fixed inset-0 z-[70] bg-black/60 backdrop-blur-sm flex justify-center items-end">
          <div className="bg-white w-full max-w-md rounded-t-[40px] p-6 pb-10 shadow-2xl animate-in slide-in-from-bottom-10">
            <div className="flex justify-between items-center mb-6 px-2">
              ...
              <button onClick={() => setIsAvatarMenuOpen(false)} className="p-2 bg-gray-100 rounded-full text-brand-dark hover:bg-gray-200 transition">
            ...
            <div className="flex flex-col gap-6">
              {/* רשימת חיות למעלה */}
              <div className="bg-gray-50/50 p-5 rounded-[32px] border border-gray-100 shadow-inner">
                <p className="text-[11px] font-black text-brand-gray mb-4 text-center tracking-widest uppercase">דמויות החיות של שכן+</p>
                <div className="grid grid-cols-4 gap-4">
                  {animalAvatars.map((avatar, idx) => (
                    <button 
                      key={idx} 
                      onClick={() => updateAvatarInDB(avatar)} 
                      className="aspect-square rounded-2xl bg-white border-2 border-transparent hover:border-brand-blue hover:shadow-lg transition active:scale-90 overflow-hidden flex items-center justify-center p-1 shadow-sm"
                    >
              ...
              {/* כפתור גלריה למטה */}
              <button onClick={() => avatarInputRef.current?.click()} className="w-full flex items-center justify-center gap-3 bg-brand-blue text-white py-4.5 rounded-2xl font-black shadow-lg active:scale-95 transition">
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                בחירה מהגלריה במכשיר
              </button>

              <button onClick={resetToInitials} className="w-full text-brand-dark font-bold py-2 text-sm opacity-60 hover:opacity-100 transition">
                חזור לראשי תיבות
              </button>
            </div>
          </div>
