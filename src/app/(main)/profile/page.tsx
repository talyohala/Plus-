...
export default function ProfilePage() {
  ...
  if (isLoading) {
    return <div className="flex flex-col flex-1 w-full items-center justify-center pb-32"><div className="w-10 h-10 border-4 border-slate-200 border-t-[#1D4ED8] rounded-full animate-spin"></div><p className="mt-4 font-bold text-slate-800">טוען נתונים...</p></div>
  }
  ...
  return (
    <div className="flex flex-col flex-1 w-full pb-32 bg-[#F8FAFC] min-h-screen" dir="rtl">
      ...
      <div className="px-5 mb-6 mt-4 flex justify-between items-center">
        ...
        <Link href="/settings" className="p-2.5 bg-white rounded-full border border-slate-200 shadow-sm text-slate-500 hover:text-[#1D4ED8] transition active:scale-95">
        ...
        <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-[0_4px_20px_rgb(0,0,0,0.03)] flex flex-col gap-5">
          <div className="flex items-center gap-4">
            ...
            <div onClick={() => setIsAvatarMenuOpen(true)} className="relative w-24 h-24 shrink-0 cursor-pointer group block">
              <div className="w-full h-full rounded-[1.5rem] border-4 border-white bg-[#E3F2FD] shadow-sm overflow-hidden relative flex items-center justify-center">
            ...
            <div className="flex-1 pt-1">
              <input
                ...
                className="text-xl font-black text-slate-800 leading-tight bg-transparent outline-none w-full border-b border-dashed border-slate-200 focus:border-[#1D4ED8] transition-colors pb-1"
                placeholder="שם מלא"
              />
          ...
          <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 flex flex-col gap-4 shadow-inner">
            <div className="flex gap-3">
              <div className="w-full">
                <label className="text-[10px] font-bold text-slate-500 mb-1 block uppercase">מספר דירה</label>
                <input type="text" value={apartment} onChange={e => setApartment(e.target.value)} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-[#1D4ED8] text-slate-800 shadow-sm transition" placeholder="מספר דירה" />
              </div>
              <div className="w-full">
                <label className="text-[10px] font-bold text-slate-500 mb-1 block uppercase">קומה</label>
                <input type="text" value={floor} onChange={e => setFloor(e.target.value)} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-[#1D4ED8] text-slate-800 shadow-sm transition" placeholder="מספר קומה" />
              </div>
            </div>
            <button onClick={updatePersonalDetails} disabled={isUpdating} className="w-full bg-slate-800 text-white text-xs font-bold py-3.5 rounded-xl hover:bg-slate-900 active:scale-95 transition shadow-sm disabled:opacity-50">
      ...
      {isPending && building && (
        <div className="px-5 mb-8">
          <div className="bg-yellow-50 border border-yellow-200 rounded-[2rem] p-6 shadow-sm text-center">
            ...
            <p className="text-sm text-slate-600 mb-0">בקשתך להצטרף לקהילת <strong>{building.name}</strong> נשלחה למנהל הוועד. הגישה תפתח מיד עם אישורו.</p>
          </div>
        </div>
      )}

      {/* המצב הקודם - אם אין בניין (הצטרפות או יצירה) */}
      {!building && !isPending && (
        <div className="px-5 space-y-4 mb-8">
          <div className="bg-white border border-slate-100 rounded-[2rem] p-6 shadow-[0_4px_20px_rgb(0,0,0,0.02)]">
            ...
            <div className="flex flex-col gap-3">
              <input
                ...
                className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3.5 text-sm outline-none focus:border-[#1D4ED8] text-slate-800 text-center tracking-widest font-mono font-bold uppercase transition"
          ...
          <div className="flex items-center justify-center gap-4 py-2 opacity-50">
            ...
            <span className="text-xs font-bold text-slate-500">או פתיחת קהילה חדשה</span>
            <div className="h-px bg-slate-300 flex-1"></div>
          </div>
          ...
          <div className="bg-slate-800 border border-slate-700 rounded-[2rem] p-6 shadow-xl relative overflow-hidden text-white">
            ...
            <p className="text-sm text-slate-300 mb-4 relative z-10">אתה ועד הבית? פתח קהילה חדשה ונהל את אישורי השכנים והתשלומים.</p>
            <div className="flex flex-col gap-3 relative z-10">
              <input
                type="text"
                value={createBuildingName}
                onChange={(e) => setCreateBuildingName(e.target.value)}
                className="w-full bg-white/10 border border-white/20 rounded-xl px-4 py-3.5 text-sm outline-none focus:border-white text-white placeholder-slate-400 transition"
      ...
      {/* אזור ניהול קהילה (לוועד בלבד) - בדיוק כמו שהיה */}
      {building && !isPending && (
        <div className="px-5 space-y-6">
          <section className="bg-white border border-slate-100 rounded-[2rem] p-5 shadow-[0_4px_20px_rgb(0,0,0,0.02)]">
            ...
            {isAdmin ? (
              <div className="flex flex-col gap-3">
                <input
                  ...
                  className="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-3.5 text-sm outline-none focus:border-[#1D4ED8] transition text-slate-800 font-bold shadow-inner"
                />
                <button onClick={updateBuildingName} disabled={isUpdating || newBuildingName === building.name} className="w-full bg-[#E3F2FD] text-[#1D4ED8] px-5 py-3 rounded-xl text-xs font-bold active:scale-95 transition disabled:opacity-50">
                  {isUpdating ? 'שומר...' : 'עדכן את שם הקהילה'}
                </button>
              </div>
            ) : (
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 text-center font-bold text-slate-800">
                {building.name}
              </div>
            )}
            ...
            <div className="mt-4 pt-4 border-t border-slate-100 flex items-center justify-between px-1">
              <div>
                <p className="text-[10px] font-bold text-slate-400 mb-1 uppercase tracking-wider">קוד זיהוי להצטרפות שכנים</p>
                <p className="text-lg font-black font-mono text-slate-800 tracking-widest">{shortCode}</p>
              </div>
              <button onClick={copyBuildingCode} className="text-xs font-bold text-[#1D4ED8] bg-[#E3F2FD] px-4 py-2.5 rounded-xl active:scale-95 transition">
                העתקת קוד
              </button>
            </div>
          </section>
                ...
                {pendingNeighbors.map((n) => (
                  <div key={n.id} className="flex items-center justify-between p-4 border-b border-orange-50 last:border-0">
                    <div className="flex items-center gap-3">
                      <img src={n.avatar_url || `https://api.dicebear.com/8.x/initials/svg?seed=${n.full_name}&backgroundColor=eef2ff&textColor=1e3a8a`} className="w-10 h-10 rounded-[12px] border border-slate-100 shadow-sm" />
                      <div>
                    ...
                    <div className="flex gap-2">
                      ...
                      <button onClick={() => approveNeighbor(n.id)} className="w-9 h-9 rounded-xl bg-green-500 text-white shadow-md flex items-center justify-center active:scale-95 transition">
          ...
          {/* רשימת השכנים המאושרים במערכת (מוצג למנהל) */}
          {isAdmin && (
            <section>
                ...
                {approvedNeighbors.length === 0 ? (
                  ...
                  approvedNeighbors.map((n) => (
                    <div key={n.id} className="flex items-center justify-between p-4 border-b border-slate-50 last:border-0 hover:bg-slate-50 transition">
                      <div className="flex items-center gap-3">
                        <img src={n.avatar_url || `https://api.dicebear.com/8.x/initials/svg?seed=${n.full_name}&backgroundColor=eef2ff&textColor=1e3a8a`} className="w-10 h-10 rounded-[12px] border border-slate-100 shrink-0" />
                        <div>
      ...
      {/* תפריט שינוי תמונת פרופיל מהעיצוב הקודם הנקי */}
      {isAvatarMenuOpen && (
        <div className="fixed inset-0 z-[70] bg-black/60 backdrop-blur-sm flex justify-center items-end">
          <div className="bg-[#F8FAFC] w-full max-w-md rounded-t-[2.5rem] p-6 pb-10 shadow-2xl animate-in slide-in-from-bottom-10">
            <div className="flex justify-between items-center mb-6 px-2">
              <h3 className="font-black text-xl text-slate-800">בחירת תמונת פרופיל</h3>
              <button onClick={() => setIsAvatarMenuOpen(false)} className="p-2 bg-white rounded-full border border-slate-200 text-slate-500 hover:text-slate-800 transition active:scale-95 shadow-sm">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"></path></svg>
              </button>
            </div>
            
            <div className="flex flex-col gap-5">
              <div className="bg-[#E3F2FD]/40 p-5 rounded-[2rem] border border-[#BFDBFE]/50">
              ...
              <div className="flex flex-col gap-3 mt-2">
                <button onClick={() => avatarInputRef.current?.click()} className="w-full flex items-center justify-center gap-2 bg-white text-[#1D4ED8] border border-slate-200 py-4 rounded-2xl font-bold hover:bg-slate-50 active:scale-95 transition shadow-sm">
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                  בחירה מגלריית התמונות
                </button>
                <button onClick={resetToInitials} className="w-full flex items-center justify-center gap-2 bg-slate-100 text-slate-600 border border-slate-200 py-4 rounded-2xl font-bold hover:bg-slate-200 active:scale-95 transition shadow-sm">
