...
export default function ProfilePage() {
  ...
  useEffect(() => {
    fetchData()
    const channel = supabase.channel('profile_realtime_v13')
  ...
  const shareViaWhatsApp = () => {
    ...
    playSystemSound('click');
    const text = `היי שכנים! 🏢\nהצטרפו לאפליקציית הבניין שלנו - שכן+.\n\nקוד ההצטרפות של הבניין הוא: *${building.invite_code}*\n\nלהורדה והצטרפות:\nhttps://shechen-plus.com/join`;
    window.open(`https://wa.me/?text=${encodeURIComponent(text)}`, '_blank');
  }

  if (isLoading) {
    return <div className="flex flex-col flex-1 w-full items-center justify-center pb-32"><div className="w-10 h-10 border-4 border-gray-200 border-t-[#1D4ED8] rounded-full animate-spin"></div><p className="mt-4 font-bold text-slate-800">טוען נתונים...</p></div>
  }
  ...
  return (
    <div className="flex flex-col flex-1 w-full pb-32 bg-[#F8FAFC]" dir="rtl">
      ...
      <div className="px-4 mb-8">
        <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-[0_4px_20px_rgb(0,0,0,0.02)] flex flex-col gap-5">
          ...
          <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 flex flex-col gap-4 shadow-inner">
            <div className="flex gap-3">
              <div className="w-full">
                <label className="text-[10px] font-bold text-slate-500 mb-1 block">מספר דירה</label>
              ...
              <div className="w-full">
                <label className="text-[10px] font-bold text-slate-500 mb-1 block">קומה</label>
          ...
          <div className="bg-slate-800 border border-slate-700 rounded-[2rem] p-6 shadow-xl relative overflow-hidden text-white">
            ...
            <div className="flex flex-col gap-3">
              ...
              <button onClick={handleCreateBuilding} disabled={isUpdating || !createBuildingName.trim()} className="w-full bg-white text-slate-800 px-5 py-3.5 rounded-xl text-sm font-bold active:scale-95 transition disabled:opacity-50">
                {isUpdating ? 'מקים קהילה...' : 'הקם קהילה מנהל'}
              </button>
            </div>
          </div>
        </div>
      ...
      {building && !isPending && (
        <div className="px-4 space-y-6">
          <section className="bg-white border border-slate-100 rounded-[2rem] p-6 shadow-[0_4px_20px_rgb(0,0,0,0.02)]">
            <h4 className="text-sm font-black text-slate-800 mb-4">פרטי הקהילה</h4>
            
            {isAdmin ? (
              <div className="flex flex-col gap-3">
                <input type="text" value={newBuildingName} onChange={(e) => setNewBuildingName(e.target.value)} className="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-3.5 text-sm outline-none focus:border-[#1D4ED8] transition text-slate-800 font-bold" />
                <button onClick={updateBuildingName} disabled={isUpdating || newBuildingName === building.name} className="w-full bg-[#E3F2FD] text-[#1D4ED8] px-5 py-3.5 rounded-xl text-xs font-bold active:scale-95 transition disabled:opacity-50">
                  {isUpdating ? 'שומר...' : 'עדכן את שם הקהילה'}
                </button>
              </div>
            ) : (
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 text-center font-bold text-slate-800">
                {building.name}
              </div>
            )}
            
            {/* כרטיס הזמנת שכנים מעוצב בצבעי ה-FAB החדש */}
            {isAdmin && building.invite_code && (
              <div className="mt-5 bg-slate-600 rounded-2xl p-5 relative overflow-hidden shadow-lg">
                ...
                <div className="relative z-10 flex flex-col gap-3">
                   <div className="flex items-center justify-between">
                     <div>
                       <p className="text-[10px] text-slate-300 font-bold mb-0.5">קוד הזמנה לבניין</p>
                       <h2 className="text-2xl font-black text-white tracking-widest">{building.invite_code}</h2>
                     </div>
                     <button onClick={shareViaWhatsApp} className="w-10 h-10 bg-[#25D366] text-white rounded-full flex items-center justify-center shadow-md active:scale-95 transition">
                   ...
                   <p className="text-[11px] text-slate-300">שתפו את הקוד עם השכנים כדי להוסיף אותם לבניין.</p>
                </div>
              </div>
            )}
          </section>
          ...
          {isAdmin && (
            <section>
              <h4 className="text-[11px] font-black text-slate-400 uppercase tracking-wider mb-3 pr-1">ניהול הרשאות שכנים מאושרים ({approvedNeighbors.length})</h4>
                ...
                {approvedNeighbors.length === 0 ? (
                  ...
                  approvedNeighbors.map((n) => (
                    <div key={n.id} className="flex items-center justify-between p-4 border-b border-slate-50 last:border-0 hover:bg-slate-50 transition">
                      <div className="flex items-center gap-3">
                        <img src={n.avatar_url || `https://api.dicebear.com/8.x/initials/svg?seed=${n.full_name}&backgroundColor=eef2ff&textColor=1e3a8a`} className="w-10 h-10 rounded-full border border-slate-100 shrink-0" />
                        <div>
      ...
      {/* תפריט שינוי תמונת פרופיל (נשאר ללא שינוי) */}
      {isAvatarMenuOpen && (
        <div className="fixed inset-0 z-[70] bg-black/60 backdrop-blur-sm flex justify-center items-end">
          <div className="bg-white w-full max-w-md rounded-t-[40px] p-6 pb-10 shadow-2xl animate-in slide-in-from-bottom-10">
            ...
            <div className="flex flex-col gap-5">
              <div className="bg-[#E3F2FD]/40 p-5 rounded-[32px] border border-[#BFDBFE]/50">
      ...
      {/* הודעת פופ-אפ קצרה ומרחפת */}
      {toastMsg && (
        <div className="fixed top-20 left-1/2 -translate-x-1/2 bg-slate-600 text-white text-[11px] font-bold px-4 py-2 rounded-xl shadow-xl z-50 animate-in fade-in slide-in-from-top-4 pointer-events-none">
          {toastMsg}
        </div>
      )}
    </div>
