...
export default function ProfilePage() {
  ...
  useEffect(() => {
    fetchData()
    const channel = supabase.channel('profile_realtime_v15')
  ...
  return (
    <div className="flex flex-col flex-1 w-full pb-32 bg-[#F8FAFC] min-h-screen" dir="rtl">
      ...
      {/* כרטיס פרופיל ראשי - מעוצב בצבעים שלנו */}
      <div className="px-5 mb-8">
        <div className="bg-white p-6 rounded-[2rem] border border-[#E3F2FD] shadow-[0_8px_30px_rgb(29,78,216,0.04)] flex flex-col gap-6 relative overflow-hidden">
          <div className="absolute -top-10 -right-10 w-32 h-32 bg-[#E3F2FD] rounded-full blur-3xl opacity-50 pointer-events-none"></div>
          ...
          <div className="flex items-center gap-5 relative z-10">
            ...
            <div onClick={() => setIsAvatarMenuOpen(true)} className="relative w-[5.5rem] h-[5.5rem] shrink-0 cursor-pointer group">
              <div className="w-full h-full rounded-[1.5rem] border-4 border-white bg-[#E3F2FD] shadow-md overflow-hidden flex items-center justify-center">
                ...
                {isUpdating && <div className="absolute inset-0 bg-white/60 backdrop-blur-sm flex items-center justify-center"><div className="w-5 h-5 border-2 border-[#1D4ED8] border-t-transparent rounded-full animate-spin"></div></div>}
              </div>
            ...
            <div className="flex-1">
              <input
                ...
                placeholder="שם מלא"
              />
              <span className={`text-[10px] font-black px-3 py-1 rounded-full mt-1.5 inline-block shadow-sm ${!building ? 'bg-orange-50 text-orange-600 border border-orange-100' : isPending ? 'bg-yellow-50 text-yellow-600 border border-yellow-100' : isAdmin ? 'bg-[#E3F2FD] text-[#1D4ED8] border border-[#BFDBFE]' : 'bg-slate-50 text-slate-600 border border-slate-200'}`}>
                {!building ? 'ללא קהילה' : isPending ? 'ממתין לאישור ועד' : isAdmin ? 'מנהל הוועד' : 'דייר בבניין'}
              </span>
            </div>
          </div>

          <div className="bg-[#F8FAFC] p-4 rounded-2xl flex flex-col gap-4 relative z-10">
            <div className="flex gap-3">
              <div className="flex-1">
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1 block">דירה</label>
                <input type="text" value={apartment} onChange={e => setApartment(e.target.value)} className="w-full bg-white border border-[#E3F2FD] rounded-xl px-4 py-2.5 text-sm font-bold outline-none focus:border-[#1D4ED8] text-slate-800 shadow-sm transition" placeholder="-" />
              </div>
              <div className="flex-1">
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1 block">קומה</label>
                <input type="text" value={floor} onChange={e => setFloor(e.target.value)} className="w-full bg-white border border-[#E3F2FD] rounded-xl px-4 py-2.5 text-sm font-bold outline-none focus:border-[#1D4ED8] text-slate-800 shadow-sm transition" placeholder="-" />
              </div>
            </div>
            <button onClick={updatePersonalDetails} disabled={isUpdating} className="w-full bg-[#E3F2FD] text-[#1D4ED8] text-sm font-black py-3.5 rounded-xl hover:bg-[#BFDBFE] active:scale-95 transition shadow-sm disabled:opacity-50">
      ...
      {/* אזור בניין קיים וקוד הזמנה */}
      {building && !isPending && (
        <div className="px-5 space-y-6">
          
          {/* כרטיס קוד הזמנה - פרימיום (מוצג רק לוועד) */}
          {isAdmin && inviteCode && (
            <div className="bg-gradient-to-l from-[#1D4ED8] to-blue-600 rounded-[2rem] p-6 text-white shadow-[0_8px_25px_rgba(29,78,216,0.25)] relative overflow-hidden">
              <div className="absolute -top-10 -right-10 w-32 h-32 bg-white/10 rounded-full blur-2xl pointer-events-none"></div>
              
              <div className="relative z-10 flex justify-between items-start mb-6">
                <div>
                  <h4 className="text-sm font-black text-blue-100 mb-1">קוד הצטרפות לבניין</h4>
                  <p className="text-3xl font-black font-mono tracking-widest drop-shadow-sm">{inviteCode}</p>
                </div>
                <button onClick={copyBuildingCode} className="w-10 h-10 bg-white/10 hover:bg-white/20 rounded-full flex items-center justify-center transition active:scale-95 backdrop-blur-sm">
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"></path></svg>
                </button>
              </div>
              
              <button onClick={inviteNeighbors} className="relative z-10 w-full bg-white text-[#1D4ED8] font-black py-3.5 rounded-xl text-sm shadow-lg active:scale-95 transition flex items-center justify-center gap-2">
          ...
          {/* ניהול קהילה */}
          <section className="bg-white border border-[#E3F2FD] rounded-[2rem] p-5 shadow-[0_2px_15px_rgb(29,78,216,0.03)]">
            <div className="flex items-center justify-between mb-4">
              <h4 className="text-sm font-black text-slate-800 px-1">שם הקהילה</h4>
            </div>
          ...
          {isAdmin && (
            <section>
              ...
              <div className="bg-white border border-[#E3F2FD] rounded-[2rem] overflow-hidden shadow-[0_2px_15px_rgb(29,78,216,0.03)]">
                    ...
                    <div key={n.id} className="flex items-center justify-between p-4 border-b border-slate-50 last:border-0 hover:bg-slate-50 transition">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-bold text-slate-800 flex items-center gap-1.5">
                            {n.full_name} {n.role === 'admin' && <span className="text-[9px] bg-[#E3F2FD] text-[#1D4ED8] px-1.5 py-0.5 rounded-md">ועד</span>}
                          </p>
                          <p className="text-[10px] text-slate-500">דירה {n.apartment || '?'} | קומה {n.floor || '?'}</p>
                        </div>
                      </div>
                      {n.id !== profile.id && (
                        <button onClick={() => toggleRole(n.id, n.role)} className={`text-[10px] font-black px-3 py-2 rounded-xl transition active:scale-95 shadow-sm ${n.role === 'admin' ? 'bg-red-50 text-red-500 border border-red-100' : 'bg-[#E3F2FD] text-[#1D4ED8] border border-[#BFDBFE]'}`}>
                          {n.role === 'admin' ? 'הסר ועד' : 'מינוי לוועד'}
                        </button>
                      )}
                    </div>
                  ))
      ...
      {/* ללא בניין - יצירה או הצטרפות (נקי וברור) */}
      {!building && !isPending && (
        <div className="px-5 space-y-5 mb-8">
          <div className="bg-white border border-[#E3F2FD] rounded-[2rem] p-6 shadow-[0_4px_20px_rgb(29,78,216,0.04)]">
            ...
            <p className="text-sm text-slate-500 mb-4">יש לך קוד מהוועד? הזן אותו כאן.</p>
          ...
          <div className="bg-white border border-[#E3F2FD] rounded-[2rem] p-6 shadow-[0_4px_20px_rgb(29,78,216,0.04)] relative overflow-hidden">
        ...
        <div className="px-5 mb-8">
          <div className="bg-yellow-50 border border-yellow-200 rounded-[2rem] p-6 shadow-sm text-center">
            ...
            <p className="text-sm text-slate-500 mb-0">
              בקשתך להצטרף לקהילת <strong>{building.name}</strong> נשלחה למנהל. תעודכן מיד עם אישורו.
            </p>
          </div>
        </div>
      )}
