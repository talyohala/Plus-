...
export default function RecommendationsPage() {
  ...
  const handleAddRecommendation = async (e: React.FormEvent) => {
    ...
    if (!error) {
      ...
      fetchProfessionals() // רענון הרשימה מיד לאחר ההוספה
    } else {
  ...
  return (
    <div className="flex flex-col flex-1 w-full pb-24 relative" dir="rtl">
          ...
          professionals.map(pro => (
            <div key={pro.id} className="bg-white p-5 rounded-3xl shadow-sm border border-gray-50 flex flex-col gap-3 relative overflow-hidden">
              <div className="flex items-center gap-4">
                <img src={`https://api.dicebear.com/7.x/initials/svg?seed=${pro.name}&backgroundColor=0e1e2d`} alt={pro.name} className="w-14 h-14 rounded-2xl shadow-sm z-10" />
      ...
      {/* כפתור צף להוספת המלצה (כעת מחובר) */}
      <button onClick={() => setIsModalOpen(true)} className="fixed bottom-24 left-4 z-40 bg-brand-blue text-white p-4 rounded-2xl shadow-xl hover:scale-105 active:scale-95 transition flex items-center gap-2">
        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M12 4v16m8-8H4"></path></svg>
        <span className="font-bold text-sm pr-1">המלץ</span>
      </button>

      {/* מודל להוספת בעל מקצוע */}
      {isModalOpen && (
          ...
          <div className="bg-white w-full max-w-md rounded-t-3xl sm:rounded-3xl p-6 shadow-2xl animate-in slide-in-from-bottom-10">
            ...
            <form onSubmit={handleAddRecommendation} className="space-y-4">
              <div>
                ...
                <input type="text" required value={newPro.name} onChange={e => setNewPro({...newPro, name: e.target.value})} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-brand-blue" placeholder="לדוג': אבי חשמל" />
              </div>
              
              <div className="flex gap-4">
                <div className="flex-1">
                  <label className="text-xs font-bold text-brand-dark mb-1 block">מספר טלפון *</label>
                  <input type="tel" required value={newPro.phone} onChange={e => setNewPro({...newPro, phone: e.target.value})} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-brand-blue" placeholder="050-0000000" />
                </div>
                <div className="flex-1">
                  <label className="text-xs font-bold text-brand-dark mb-1 block">קטגוריה</label>
                  <select value={newPro.category} onChange={e => setNewPro({...newPro, category: e.target.value})} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-brand-blue">
              ...
              <div>
                ...
                <div className="flex gap-2 justify-center bg-gray-50 py-3 rounded-xl border border-gray-200">
                  {[1,2,3,4,5].map(star => (
                    <button key={star} type="button" onClick={() => setNewPro({...newPro, rating: star})} className="transition hover:scale-110">
                      <svg className={`w-8 h-8 ${star <= newPro.rating ? 'text-orange-400' : 'text-gray-300'}`} fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div>
                ...
                <textarea value={newPro.review_text} onChange={e => setNewPro({...newPro, review_text: e.target.value})} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-brand-blue min-h-[80px]" placeholder="הגיע בזמן, עבד נקי, לקח מחיר הוגן..."></textarea>
              </div>

              <button type="submit" disabled={isSubmitting} className="w-full bg-brand-blue text-white font-bold py-4 rounded-xl shadow-lg mt-2 active:scale-95 transition disabled:opacity-50">
