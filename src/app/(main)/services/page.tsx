      ...
      <div className="space-y-4 px-4">
        {tickets.length === 0 ? (
          ...
          tickets.map(ticket => {
            ...
            if (ticket.status === 'טופל') statusColor = 'bg-green-50 text-green-500 border-green-100'

            return (
              <div key={ticket.id} className="bg-white p-5 rounded-3xl shadow-sm border border-gray-50 flex flex-col relative overflow-hidden">
                
                {/* כפתור מחיקה אם הקריאה שלי */}
                {(isMine || isAdmin) && ticket.status !== 'טופל' && (
                  <button onClick={() => handleDelete(ticket.id)} className="absolute top-4 left-4 p-1.5 text-gray-300 hover:text-red-500 transition">
                ...
                <div className="flex items-start gap-3 mb-3">
                  ...
                  <div className="flex-1 pr-1">
                    <div className="flex gap-2 items-center mb-1">
                      <h3 className="font-black text-brand-dark text-base">{ticket.title}</h3>
                      {ticket.urgency === 'דחוף' && <span className="bg-red-500 text-white text-[9px] font-black px-1.5 py-0.5 rounded-md">דחוף</span>}
                    </div>
                    <p className="text-xs font-bold text-brand-dark/70 mb-1">{ticket.profiles?.full_name} • {new Date(ticket.created_at).toLocaleDateString('he-IL')}</p>
                    {ticket.description && <p className="text-sm text-brand-dark/80 leading-relaxed mb-3">{ticket.description}</p>}
                ...
                {isAdmin && ticket.status !== 'טופל' && (
                  <div className="flex gap-2 mt-3 pt-3 border-t border-gray-50">
                    {ticket.status === 'פתוח' && (
                      <button onClick={() => updateTicketStatus(ticket.id, 'בטיפול')} className="flex-1 text-[10px] font-bold bg-orange-50 text-orange-600 hover:bg-orange-100 py-2 rounded-xl transition">
                        סמן כ״בטיפול״
                      </button>
                    )}
                    <button onClick={() => updateTicketStatus(ticket.id, 'טופל')} className="flex-1 text-[10px] font-bold bg-green-50 text-green-600 hover:bg-green-100 py-2 rounded-xl transition">
                      סמן כ״טופל״
                    </button>
                  </div>
                )}
          ...
          <div className="bg-white w-full max-w-md rounded-t-3xl p-6 pb-8 shadow-2xl animate-in slide-in-from-bottom-10 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleOpenTicket} className="space-y-4">
              <div>
                ...
                <input type="text" required value={newTicket.title} onChange={e => setNewTicket({...newTicket, title: e.target.value})} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-brand-blue transition" placeholder="לדוג': נזילה בצינור בלובי" />
              </div>

              <div>
                <label className="text-xs font-bold text-brand-dark mb-1 block">דחיפות</label>
                <select value={newTicket.urgency} onChange={e => setNewTicket({...newTicket, urgency: e.target.value})} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-brand-blue transition">
              ...
              <div>
                ...
                <textarea value={newTicket.description} onChange={e => setNewTicket({...newTicket, description: e.target.value})} className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-brand-blue transition min-h-[80px]" placeholder="נא לפרט מיקום מדויק..."></textarea>
              </div>
