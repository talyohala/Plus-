...
const analyzeTicketAI = (text: string, suppliersList: any[]) => {
  ...
  // AI מנקה פתיחים ארוכים ומיותרים כדי להגיע ללב הבעיה
  let summary = text.trim();
  const prefixes = /^(שלום|היי|בוקר טוב|ערב טוב|צהריים טובים|לוועד|דיירים יקרים|שכנים|אשמח לעזרה|יש לנו|ראיתי ש|שמתי לב ש|רציתי לדווח על|רציתי לדווח|מדווח על|סליחה על ההפרעה|הודעה לוועד|דחוף|שימו לב|מישהו יכול)/gi;
  summary = summary.replace(prefixes, '').trim();
  
  // חיתוך חכם וסופר-מקצועי (עד 7 מילים, בלי לחתוך באמצע משפט/מילת קישור)
  let clause = summary.split(/[.,;!?\n|-]/)[0].trim(); // לוקח את החלק ההגיוני הראשון
  let words = clause.split(/\s+/);
  
  if (words.length > 7) {
    const badEndings = ['של', 'את', 'על', 'עם', 'או', 'וגם', 'מ', 'ל', 'ב', 'כ', 'מן', 'זה', 'כי'];
    let limit = 7;
    // מונע חיתוך שמשאיר מילת קישור תלויה באוויר
    while (limit > 3 && badEndings.includes(words[limit - 1])) {
      limit--;
    }
    clause = words.slice(0, limit).join(' ');
  }
  summary = clause.replace(/[.,;?!]+$/, '').trim(); // ניקוי פיסוק מיותר בסוף
  
  if (!summary) summary = 'תקלה מדווחת';

  // ניתוח עמוק לקטגוריות ודחיפות
  if (/(מים|נזילה|פיצוץ|סתימה|ביוב|אינסטלטור|צינור|טפטוף|הצפה|רטיבות|דוד|ברז|אסלה|כיור|מקלחת|דלף)/.test(t)) {
    category = 'אינסטלציה';
    urgency = /(פיצוץ|הצפה|דחוף|חזק|מיידי)/.test(t) ? 'high' : 'medium';
  } else if (/(חשמל|קצר|שקע|מנורה|חושך|נשרף|פחת|חשמלאי|אור|הפסקת|לוח|חוטים|שרוף|הבהוב)/.test(t)) {
    category = 'חשמל';
    urgency = /(קצר|נשרף|אש|מסוכן|גיצים|עשן|התחשמלות)/.test(t) ? 'high' : 'medium';
  } else if (/(מעלית|תקוע|לא עובדת|למעלה|למטה|מעליות|כפתור במעלית|פיר)/.test(t)) {
    category = 'מעליות';
    urgency = 'high';
  } else if (/(ניקיון|מלוכלך|פח|זבל|לנקות|ריח|מסריח|שטיפה|כתם|אבק|ספונג'ה|פחים)/.test(t)) {
    category = 'ניקיון';
    urgency = 'low';
  } else if (/(גינה|עץ|השקיה|דשא|צמחים|עציץ|גינון|עלים|פרחים|שתילים|עשבים)/.test(t)) {
    category = 'גינון';
    urgency = 'low';
  } else if (/(דלת|שער|מנעול|אינטרקום|קודן|חניה|שלט|מפתח|לא נפתח|תקועה|זגוגית|ציר)/.test(t)) {
    category = 'שערים ודלתות';
    urgency = /(לא נפתח|תקוע|רכב בפנים)/.test(t) ? 'high' : 'medium';
  } else if (/(גז|בלון|ריח של גז|דליפה)/.test(t)) {
    category = 'גז';
    urgency = 'high';
  }

  // שיוך ספק חכם (עדיפות לספק הבית)
  ...
  if (relevantSuppliers.length > 0) {
    ...
    const recSups = relevantSuppliers.filter(s => s.type === 'recommended').sort((a,b) => b.rating - a.rating);
    assignedSupplier = houseSups.length > 0 ? houseSups[0] : recSups[0];
  }
...
export default function ServicesPage() {
  ...
  const handleUpdateStatus = async (id: string, newStatus: string) => {
    ...
    if (ticket && ticket.user_id && ticket.user_id !== profile.id) {
      await supabase.from('notifications').insert([{
        ...
        content: `הוועד עדכן את הסטטוס של התקלה שלך ל-${newStatus}.`,
        link: '/services'
      }]);
    }
    fetchData();
  ...
  const handleTogglePin = async (ticket: any) => {
    playSystemSound('click');
    setOpenMenuId(null);
    const newPinnedStatus = !ticket.is_pinned; 
    await supabase.from('service_tickets').update({ is_pinned: newPinnedStatus }).eq('id', ticket.id);
    fetchData();
  };
  ...
  const handleShareWhatsAppTicket = (ticket: any) => {
    playSystemSound('click');
    setOpenMenuId(null);
    const analysis = analyzeTicketAI(ticket.description || ticket.title || '', suppliers);
    const text = `*תקלה בבניין:*\n${ticket.title || analysis.summary}\n\n*קטגוריה:* ${analysis.category}\n*סטטוס:* ${ticket.status}\n\n*תיאור:*\n${ticket.description}\n\nנפתח ע"י: ${ticket.profiles?.full_name}`;
  ...
  const handleSubmitTicket = async (e: React.FormEvent) => {
    ...
    const analysis = analyzeTicketAI(newTicketText, suppliers);
    
    const { error } = await supabase.from('service_tickets').insert([{
      building_id: profile.building_id,
      user_id: profile.id,
      title: analysis.summary,
      description: newTicketText,
      status: 'פתוח'
    }]);
    ...
    if (error) {
      console.error(error);
      setCustomAlert({ title: 'שגיאה', message: 'מערכת דיווח התקלות אינה זמינה כרגע, אנא נסה שוב מאוחר יותר.', type: 'error' });
    } else {
      setCustomAlert({ title: 'הדיווח התקבל!', message: `התקלה סווגה ונותבה בהצלחה כ-${analysis.category}.`, type: 'success' });
      setNewTicketText('');
  ...
  const handleAddSupplier = (e: React.FormEvent) => {
    ...
    setMyRatings(prev => ({...prev, [newId]: newSupplier.initialRating}));
    
    setCustomAlert({ title: 'הספק נוסף בהצלחה', message: 'תודה! הוספת איש מקצוע עוזרת לכלל דיירי שכן+.', type: 'success' });
    setNewSupplier({ name: '', category: 'תחזוקה כללית', phone: '', initialRating: 5 });
    setIsSupplierModalOpen(false);
    playSystemSound('success');
  };

  const handleRateSupplier = (supplierId: string, rating: number) => {
    playSystemSound('click');
    setMyRatings(prev => ({...prev, [supplierId]: rating}));
    setCustomAlert({ title: 'הדירוג נקלט', message: 'תודה! הדירוג שלך שומר על איכות ואמינות נותני השירות.', type: 'success' });
  };
  ...
  const alertsPortal = mounted && customAlert ? createPortal(
    <div className="fixed inset-0 z-[10000] bg-black/40 backdrop-blur-sm flex justify-center items-center p-4" onClick={() => setCustomAlert(null)} dir="rtl">
      <div className="bg-white/95 backdrop-blur-xl rounded-[1.5rem] p-6 w-full max-w-sm shadow-2xl text-center animate-in zoom-in-95 border border-white/50" onClick={e => e.stopPropagation()}>
  ...
  return (
    <div className="flex flex-col flex-1 w-full pb-32 relative bg-transparent min-h-[100dvh]" dir="rtl" onClick={() => setOpenMenuId(null)}>
      ...
      <div className="px-5 mb-6">
        <div className="flex bg-white/60 backdrop-blur-md p-1.5 rounded-full border border-[#1D4ED8]/10 shadow-sm relative z-10">
          {['פתוחות', 'בטיפול', 'היסטוריה', 'ספקים'].map(tab => (
            <button key={tab} onClick={(e) => { e.stopPropagation(); setActiveTab(tab); }} className={`flex-1 h-10 rounded-full text-[13px] transition-all flex items-center justify-center font-bold ${activeTab === tab ? 'text-[#1D4ED8] bg-blue-50 border border-blue-100 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
              {tab}
            </button>
          ))}
        </div>
          ...
          isLoading ? (
            ...
            filteredTickets.map(ticket => {
              ...
              <div key={ticket.id} className={`bg-white/90 backdrop-blur-xl rounded-[2rem] p-5 mb-5 relative transition-all duration-300 ${ticket.is_pinned ? 'border-2 border-[#1D4ED8] shadow-[0_0_15px_rgba(29,78,216,0.4)]' : 'border border-[#1D4ED8]/10 shadow-[0_8px_30px_rgba(29,78,216,0.04)]'} ${openMenuId === ticket.id ? 'z-50' : 'z-10'}`}>
                ...
                {/* Connected Top Badges - Status + AI Category + Urgency */}
                <div className="absolute top-0 right-0 flex overflow-hidden rounded-bl-[1.5rem] rounded-tr-[2rem] shadow-sm z-10 border-b border-l border-white/20">
                  <div className={`px-4 py-1.5 text-white text-[10px] font-black ${
                    ticket.status === 'פתוח' ? 'bg-[#1D4ED8]' :
                    ticket.status === 'בטיפול' ? 'bg-orange-500' :
                    'bg-[#10B981]'
                  }`}>
                    {ticket.status}
                  </div>
                  <div className="px-3 py-1.5 bg-blue-50 text-[#1D4ED8] text-[10px] font-black border-r border-[#1D4ED8]/10">
                    {ticket.category || ticketAnalysis.category}
                  </div>
                  {(ticket.urgency === 'high' || ticketAnalysis.urgency === 'high') && (
                    <div className="px-3 py-1.5 bg-rose-50 text-rose-600 text-[10px] font-black border-r border-rose-100/50 animate-pulse">
                      דחוף
                    </div>
                  )}
                </div>
                ...
                <div className="flex justify-between items-start pt-7 pr-1">
                  ...
                  {/* Three Dots Naked Menu */}
                  {isAdmin && (
                    <div className="relative ml-1">
                      <button onClick={(e) => { e.stopPropagation(); setOpenMenuId(openMenuId === ticket.id ? null : ticket.id); }} className="p-2 flex items-center justify-center text-slate-400 hover:text-[#1D4ED8] transition-colors active:scale-95">
                      ...
                      {openMenuId === ticket.id && (
                        <>
                          ...
                          <div className="absolute left-0 top-10 w-[170px] bg-white/95 backdrop-blur-xl border border-slate-100 shadow-[0_15px_50px_rgba(0,0,0,0.15)] rounded-2xl z-[150] py-1.5 animate-in zoom-in-95">
                            ...
                            <button onClick={() => handleShareWhatsAppTicket(ticket)} className="w-full text-right px-4 h-11 text-xs font-bold text-slate-700 hover:bg-slate-50 flex items-center gap-3 border-b border-slate-100/50">
                            ...
                            <button onClick={() => handleTogglePin(ticket)} className="w-full text-right px-4 h-11 text-xs font-bold text-slate-700 hover:bg-slate-50 flex items-center gap-3 border-b border-slate-100/50">
                              <svg className="w-4 h-4 text-[#1D4ED8]" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M5 5a2 2 0 012-2h10a2 2 0 012 2v16l-7-3.5L5 21V5z"></path></svg>
                              {ticket.is_pinned ? 'ביטול נעיצה' : 'נעץ הודעה'}
                            </button>

                            {ticket.status === 'פתוח' && (
                              <button onClick={() => handleUpdateStatus(ticket.id, 'בטיפול')} className="w-full text-right px-4 h-11 text-xs font-bold text-slate-700 hover:bg-slate-50 flex items-center gap-3 border-b border-slate-100/50">
                                <svg className="w-4 h-4 text-orange-500" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                סמן בטיפול
                              </button>
                            )}
                            {ticket.status !== 'טופל' && (
                              <button onClick={() => handleUpdateStatus(ticket.id, 'טופל')} className="w-full text-right px-4 h-11 text-xs font-bold text-slate-700 hover:bg-slate-50 flex items-center gap-3 border-b border-slate-100/50">
                                <svg className="w-4 h-4 text-[#10B981]" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7"></path></svg>
                                סמן כטופל
                              </button>
                            )}
                            <button onClick={() => handleDeleteTicket(ticket.id)} className="w-full text-right px-4 h-11 text-xs font-bold text-slate-700 hover:bg-rose-50 flex items-center gap-3">
                              <svg className="w-4 h-4 text-rose-500" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                              מחיקת תקלה
                            </button>
                          </div>
                        </>
                ...
                {/* AI Assigned Supplier Section (Clean version) */}
      ...
      {activeTab !== 'היסטוריה' && (
        ...
        >
          <div className="bg-[#1D4ED8] text-white w-10 h-10 flex items-center justify-center rounded-full shadow-md font-black text-xl">
            ＋
          </div>
      ...
      {isTicketModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[9999] flex items-end justify-center">
          <div className="bg-white w-full rounded-t-[2.5rem] p-6 pb-12 shadow-2xl animate-in slide-in-from-bottom-full border-t border-white/20">
            <div className="w-12 h-1.5 bg-slate-200 rounded-full mx-auto mb-8"></div>
            <div className="flex justify-between items-center mb-6 px-1">
              <h2 className="text-2xl font-black text-slate-800">דיווח תקלה</h2>
              <button type="button" onClick={() => setIsTicketModalOpen(false)} className="w-10 h-10 flex items-center justify-center bg-slate-50 rounded-full text-slate-400 hover:bg-slate-100 hover:text-slate-800 transition-colors border border-slate-100 shadow-sm active:scale-95">
            ...
            <form onSubmit={handleSubmitTicket} className="space-y-4">
              <div className="relative">
                <textarea 
                  ...
                  placeholder="תאר את התקלה והמיקום המדויק בבניין..."
                  className="w-full bg-[#F8FAFC] border border-[#1D4ED8]/20 rounded-[1.5rem] p-5 text-sm font-bold outline-none focus:border-[#1D4ED8] shadow-inner resize-none min-h-[140px] transition-all"
                />
              </div>

              <button disabled={!newTicketText.trim() || isSubmitting} type="submit" className="w-full h-14 flex items-center justify-center bg-[#1D4ED8] text-white font-black rounded-2xl shadow-[0_8px_25px_rgba(29,78,216,0.3)] active:scale-[0.98] transition-all mt-2 text-lg">
                {isSubmitting ? 'שולח...' : 'שלח דיווח לוועד'}
              </button>
            </form>
          </div>
        </div>
      ...
      {isSupplierModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[9999] flex items-end justify-center">
          <div className="bg-white w-full rounded-t-[2.5rem] p-6 pb-12 shadow-2xl animate-in slide-in-from-bottom-full border-t border-white/20">
            <div className="w-12 h-1.5 bg-slate-200 rounded-full mx-auto mb-8"></div>
            <div className="flex justify-between items-center mb-6 px-1">
              <h2 className="text-2xl font-black text-slate-800">הוספת איש מקצוע</h2>
              <button type="button" onClick={() => setIsSupplierModalOpen(false)} className="w-10 h-10 flex items-center justify-center bg-slate-50 rounded-full text-slate-400 hover:bg-slate-100 hover:text-slate-800 transition-colors border border-slate-100 shadow-sm active:scale-95">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M6 18L18 6M6 6l12 12"></path></svg>
              </button>
            </div>
            ...
            <form onSubmit={handleAddSupplier} className="space-y-4">
              <div>
                <label className="block text-xs font-black text-slate-500 mb-1.5 px-1">שם איש המקצוע / חברה</label>
                <input required type="text" value={newSupplier.name} onChange={e => setNewSupplier({...newSupplier, name: e.target.value})} className="w-full h-14 bg-slate-50 border border-[#1D4ED8]/20 rounded-2xl px-4 text-sm font-bold outline-none focus:border-[#1D4ED8] shadow-inner" />
              </div>
              
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-black text-slate-500 mb-1.5 px-1">קטגוריה</label>
                  <select value={newSupplier.category} onChange={e => setNewSupplier({...newSupplier, category: e.target.value})} className="w-full h-14 bg-slate-50 border border-[#1D4ED8]/20 rounded-2xl px-4 text-sm font-bold outline-none focus:border-[#1D4ED8] shadow-inner">
                    <option>אינסטלציה</option>
                    <option>חשמל</option>
                    <option>מעליות</option>
                    <option>ניקיון</option>
                    <option>גינון</option>
                    <option>שערים ודלתות</option>
                    <option>תחזוקה כללית</option>
                  </select>
                </div>
                <div>
                  <label className="block text-xs font-black text-slate-500 mb-1.5 px-1">מספר טלפון</label>
                  <input required type="tel" value={newSupplier.phone} onChange={e => setNewSupplier({...newSupplier, phone: e.target.value})} dir="ltr" className="w-full h-14 bg-slate-50 border border-[#1D4ED8]/20 rounded-2xl px-4 text-sm font-bold outline-none focus:border-[#1D4ED8] text-right shadow-inner" />
                </div>
              </div>

              <div className="bg-slate-50/80 p-4 rounded-2xl border border-[#1D4ED8]/10 flex flex-col items-center justify-center gap-2 mt-2 shadow-sm">
              ...
              <button type="submit" className="w-full h-14 flex items-center justify-center bg-[#1D4ED8] text-white font-black rounded-2xl shadow-[0_8px_25px_rgba(29,78,216,0.3)] active:scale-[0.98] transition-all mt-4 text-base">
                הוסף ושמור איש מקצוע
              </button>
            </form>
          </div>
        </div>
