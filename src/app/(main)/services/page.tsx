    ...
    <div className="flex flex-col flex-1 w-full pb-24" dir="rtl">
      ...
      <div className="px-4 mb-4 mt-4 flex justify-between items-center">
        <div className="flex items-center gap-3">
          <img src={profile?.avatar_url || `https://api.dicebear.com/8.x/initials/svg?seed=${profile?.full_name}&backgroundColor=eef2ff&textColor=1e3a8a`} className="w-10 h-10 rounded-full border border-gray-200 shadow-sm object-cover" alt="פרופיל" />
        ...
        {!isReporting ? (
          <button onClick={() => setIsReporting(true)} className="w-full bg-white border border-gray-100 rounded-[2rem] p-6 shadow-sm flex items-center gap-4 active:scale-95 transition">
            ...
            <div className="text-right flex-1">
              ...
              <p className="text-sm text-gray-500">המערכת החכמה שלנו תזהה את הבעיה לבד</p>
            </div>
          </button>
        ) : (
          ...
          tickets.map(ticket => (
            <div key={ticket.id} className="bg-white p-5 rounded-3xl shadow-sm border border-gray-100 flex flex-col gap-3 relative overflow-hidden">
              ...
              <p className="text-sm font-bold text-brand-dark pr-2 flex items-center gap-1.5">
                {ticket.title === 'זיהוי חכם מופעל...' ? <span className="animate-pulse text-[#1D4ED8]">{ticket.title}</span> : ticket.title !== ticket.description ? ticket.title : null}
              </p>
              {ticket.description !== ticket.title && <p className="text-xs text-gray-600 pr-2 leading-relaxed">{ticket.description}</p>}
