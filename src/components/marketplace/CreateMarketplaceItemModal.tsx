...
export default function CreateMarketplaceItemModal({
  ...
  const handlePostSubmit = async (e: React.FormEvent) => {
    ...
    if (type === 'request') {
      ...
      await onSubmitPost({ title, description, price: isFree ? 0 : (parseFloat(price) || 0), contact_phone: defaultPhone, category, file: media?.file || null, type: media?.type || 'image' });
    }
  };
  ...
  const isPriceHidden = ['למסירה', 'חבילות ודואר', 'השאלות כלים', 'בקשות שכנים'].includes(category);
      ...
      <form onSubmit={handlePostSubmit} className="space-y-4">
        {type === 'post' && (
          <div className="flex gap-2 mb-2 overflow-x-auto hide-scrollbar pb-1">
            {mainCategories.filter(c => c !== 'הכל' && c !== 'שמורים' && c !== 'קהילה').map(c => (
        ...
        <textarea autoFocus rows={3} value={description} onChange={(e) => setDescription(e.target.value)} className="w-full bg-[#F8FAFC] border border-slate-200 rounded-2xl p-4 text-sm font-bold outline-none focus:border-[#1D4ED8] resize-none shadow-inner" placeholder={type === 'request' ? 'איזו עזרה תצטרך?' : 'כתוב כאן מה תרצה לפרסם...'} />
        ...
        <div className="flex gap-2">
          ...
          <input required type="text" value={title} onChange={(e) => setTitle(e.target.value)} className="flex-1 bg-[#F8FAFC] border border-slate-200 rounded-2xl px-4 text-sm font-bold outline-none focus:border-[#1D4ED8] shadow-inner" placeholder="כותרת" />
        </div>
        ...
        <button type="submit" disabled={isSubmitting || !title.trim() || !description.trim()} className="w-full h-14 mt-2 bg-[#1D4ED8] text-white font-black rounded-2xl shadow-lg active:scale-95 transition disabled:opacity-50 text-lg">
