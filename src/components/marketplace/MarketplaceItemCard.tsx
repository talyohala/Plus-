...
export default function MarketplaceItemCard({ item, currentUserId, isAdmin, isSaved, openMenuId, editingItemId, editItemData, isSubmitting, onToggleMenu, onToggleSave, onTogglePin, onStartEdit, onCancelEdit, onDelete, onMediaClick, onResolveItem, onAddComment, onVote, formatWhatsApp, timeFormat }: Props) {
  ...
  // חישוב ישיר מתוך האובייקט שמגיע מ-SWR - אין יותר Local State מבלבל!
  ...
  return (
    <div className={`bg-white/90 backdrop-blur-xl rounded-[2rem] pt-5 pb-4 relative transition-all duration-300 overflow-hidden ${item.is_pinned ? 'border border-orange-200/60 bg-gradient-to-br from-orange-50/80 to-white shadow-[0_8px_25px_rgba(249,115,22,0.15)]' : 'border border-slate-100 shadow-[0_8px_30px_rgba(29,78,216,0.04)]'} ${openMenuId === item.id ? 'z-50' : 'z-10'}`} dir="rtl">
      ...
      <div className="absolute top-3 left-4 z-20 flex items-center gap-3">
        ...
        <button onClick={(e) => { e.stopPropagation(); onToggleMenu(openMenuId === item.id ? null : item.id); }} className="w-8 h-8 flex items-center justify-center rounded-full text-slate-500 hover:text-[#1D4ED8] bg-white/80 backdrop-blur-md border border-slate-200 shadow-sm transition-colors active:scale-95">
          ...
          <>
            ...
            <div className="absolute left-0 top-12 w-[210px] bg-white/95 backdrop-blur-xl border border-slate-100 shadow-[0_15px_50px_rgba(0,0,0,0.15)] rounded-2xl z-[150] py-2 animate-in zoom-in-95 overflow-hidden">
              <button onClick={(e) => { e.stopPropagation(); navigator.clipboard.writeText(`${item.title}\n${item.description || ''}`); onToggleMenu(null); }} className="w-full text-right px-5 h-12 text-sm font-bold text-slate-700 hover:bg-slate-50 flex items-center justify-start gap-4 transition-colors border-b border-slate-100/50">
                <svg className="w-5 h-5 text-slate-500 shrink-0" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"></path></svg><span>העתק פרטים</span>
              </button>
              <button onClick={(e) => { e.stopPropagation(); window.open(`https://wa.me/?text=${encodeURIComponent(`*${item.title}*\n${item.description || ''}`)}`, '_blank'); onToggleMenu(null); }} className="w-full text-right px-5 h-12 text-sm font-bold text-slate-700 hover:bg-slate-50 flex items-center justify-start gap-4 transition-colors border-b border-slate-100/50">
                <svg className="w-5 h-5 text-[#25D366] fill-[#25D366] shrink-0" viewBox="0 0 24 24"><path d="M.057 24l1.687-6.163c-1.041-1.804-1.588-3.849-1.587-5.946.003-6.556 5.338-11.891 11.893-11.891 3.181.001 6.167 1.24 8.413 3.488 2.245 2.248 3.481 5.236 3.48 8.414-.003 6.557-5.338 11.892-11.893 11.892-1.99-.001-3.951-.5-5.688-1.448l-6.305 1.654zm6.597-3.807c1.676.995 3.276 1.591 5.392 1.592 5.448 0 9.886-4.434 9.889-9.885.002-5.462-4.415-9.89-9.881-9.892-5.452 0-9.887 4.434-9.889 9.884-.001 2.225.651 3.891 1.746 5.634l-.999 3.648 3.742-.981zm11.387-5.464c-.074-.124-.272-.198-.57-.347-.297-.149-1.758-.868-2.031-.967-.272-.099-.47-.149-.669.149-.198.297-.768.967-.941 1.165-.173.198-.347.223-.644.074-.297-.149-1.255-.462-2.39-1.305-.883-.653-1.48-1.459-1.653-1.758-.173-.297-.018-.458.13-.606.134-.133.297-.347.446-.521.151-.172.2-.296.3-.495.099-.198.05-.372-.025-.521-.075-.148-.669-1.611-.916-2.206-.242-.579-.487-.501-.669-.51l-.57-.01c-.198 0-.52.074-.792.372s-1.04 1.016-1.04 2.479 1.065 2.876 1.213 3.074c.149.198 2.095 3.2 5.076 4.487.709.306 1.263.489 1.694.626.712.226 1.36.194 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.695.248-1.29.173-1.413z"/></svg><span>שיתוף לוואטסאפ</span>
              </button>
              <button onClick={(e) => onToggleSave(e, item.id, isSaved)} className="w-full text-right px-5 h-12 text-sm font-bold text-slate-700 hover:bg-slate-50 flex items-center justify-start gap-4 transition-colors border-b border-slate-100/50">
                {isSaved ? <svg className="w-5 h-5 text-rose-500 fill-rose-500 shrink-0" viewBox="0 0 24 24"><path d="M11.645 20.91l-.007-.003-.022-.012a15.247 15.247 0 01-.383-.218 25.18 25.18 0 01-4.244-3.17C4.688 15.36 2.25 12.174 2.25 8.25 2.25 5.322 4.714 3 7.688 3A5.5 5.5 0 0112 5.052 5.5 5.5 0 0116.313 3c2.973 0 5.437 2.322 5.437 5.25 0 3.925-2.438 7.111-4.739 9.256a25.175 25.175 0 01-4.244 3.17 15.247 15.247 0 01-.383.219l-.022.012-.007.004-.003.001a.752.752 0 01-.704 0l-.003-.001z"/></svg> : <svg className="w-5 h-5 text-slate-500 shrink-0" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z"/></svg>}<span>{isSaved ? 'הסר משמורים' : 'שמור מודעה'}</span>
              </button>
              {isAdmin && (
                <button onClick={() => onTogglePin(item.id, item.is_pinned)} className="w-full text-right px-5 h-12 text-sm font-bold text-slate-700 hover:bg-slate-50 flex items-center justify-start gap-4 transition-colors border-b border-slate-100/50">
                  <svg className={`w-5 h-5 shrink-0 ${item.is_pinned ? 'text-[#F59E0B] fill-[#F59E0B]' : 'text-[#1D4ED8]'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M16 11V5.5L17.5 4V3H6.5V4L8 5.5V11L6 14V15H11V21H13V15H18V14L16 11Z" fill={item.is_pinned ? "currentColor" : "none"} /></svg>
                  <span>{item.is_pinned ? 'בטל נעיצה' : 'נעץ אירוע'}</span>
                </button>
              )}
              {(isAdmin || isOwner) && (
                <>
                  <button onClick={() => onStartEdit(item)} className="w-full text-right px-5 h-12 text-sm font-bold text-slate-700 hover:bg-slate-50 flex items-center justify-start gap-4 transition-colors border-b border-slate-100/50"><svg className="w-5 h-5 text-slate-500 shrink-0" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg><span>עריכת פרטים</span></button>
                  {isPackage && onResolveItem && <button onClick={() => onResolveItem(item.id)} className="w-full text-right px-5 h-12 text-sm font-bold text-slate-700 hover:bg-slate-50 flex items-center justify-start gap-4 transition-colors border-b border-slate-100/50"><svg className="w-5 h-5 text-slate-500 shrink-0" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7"></path></svg><span>סמן שנאסף</span></button>}
                  <button onClick={() => onDelete(item.id)} className="w-full text-right px-5 h-12 text-sm font-bold text-rose-500 hover:bg-red-50 flex items-center justify-start gap-4 mt-1 pt-1 transition-colors"><svg className="w-5 h-5 text-rose-500 shrink-0" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg><span>מחיקה</span></button>
                </>
              )}
            </div>
          </>
      ...
      {item.media_url && (
        <div className="mt-2 mb-4 px-3 w-full">
          <div className={`w-full aspect-video rounded-2xl flex items-center justify-center cursor-pointer overflow-hidden shadow-sm border border-slate-200 relative transition-all duration-500 ${imgLoaded ? 'bg-slate-50' : 'bg-slate-200 animate-pulse'}`} onClick={() => onMediaClick(item.media_url!, item.media_type || 'image')}>
            {item.media_type === 'video' ? (
              <div className="relative w-full h-full flex items-center justify-center">
                ...
                <div className="absolute inset-0 bg-black/10 flex items-center justify-center"><div className="w-12 h-12 bg-white/90 backdrop-blur-sm rounded-full flex items-center justify-center shadow-lg"><svg className="w-5 h-5 text-[#1D4ED8] ml-1" fill="currentColor" viewBox="0 0 20 20"><path d="M8 5v10l7-5-7-5z" /></svg></div></div>
              </div>
            ) : (
              <img src={item.media_url} className={`absolute inset-0 w-full h-full object-cover transition-opacity duration-300 ${imgLoaded ? 'opacity-100' : 'opacity-0'}`} alt="media" onLoad={() => setImgLoaded(true)} />
            )}
          </div>
        </div>
      )}
        ...
        <div className="flex justify-between items-center mb-4 px-4 mt-2">
          ...
          {showContact ? (
            <div className="flex gap-2">
              <a href={`tel:${item.contact_phone}`} onClick={(e) => e.stopPropagation()} className="w-10 h-10 rounded-xl bg-[#1D4ED8] flex items-center justify-center text-white shadow-md hover:scale-105 active:scale-95 transition-all" aria-label="חייג לדייר"><svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path d="M2 3a1 1 0 011-1h2.153a1 1 0 01.986.836l.74 4.435a1 1 0 01-.54 1.06l-1.548.773a11.037 11.037 0 006.105 6.105l.774-1.548a1 1 0 011.059-.54l4.435.74a1 1 0 01.836.986V17a1 1 0 01-1 1h-2C7.82 18 2 12.18 2 5V3z"></path></svg></a>
              <button onClick={(e) => { e.stopPropagation(); window.open(formatWhatsApp(item.contact_phone), '_blank'); }} className="w-10 h-10 rounded-xl bg-[#25D366] flex items-center justify-center text-white shadow-md hover:scale-105 active:scale-95 transition-all" aria-label="שלח וואטסאפ"><svg className="w-6 h-6 fill-current" viewBox="0 0 24 24"><path d="M.057 24l1.687-6.163c-1.041-1.804-1.588-3.849-1.587-5.946.003-6.556 5.338-11.891 11.893-11.891 3.181.001 6.167 1.24 8.413 3.488 2.245 2.248 3.481 5.236 3.48 8.414-.003 6.557-5.338 11.892-11.893 11.892-1.99-.001-3.951-.5-5.688-1.448l-6.305 1.654zm6.597-3.807c1.676.995 3.276 1.591 5.392 1.592 5.448 0 9.886-4.434 9.889-9.885.002-5.462-4.415-9.89-9.881-9.892-5.452 0-9.887 4.434-9.889 9.884-.001 2.225.651 3.891 1.746 5.634l-.999 3.648 3.742-.981zm11.387-5.464c-.074-.124-.272-.198-.57-.347-.297-.149-1.758-.868-2.031-.967-.272-.099-.47-.149-.669.149-.198.297-.768.967-.941 1.165-.173.198-.347.223-.644.074-.297-.149-1.255-.462-2.39-1.305-.883-.653-1.48-1.459-1.653-1.758-.173-.297-.018-.458.13-.606.134-.133.297-.347.446-.521.151-.172.2-.296.3-.495.099-.198.05-.372-.025-.521-.075-.148-.669-1.611-.916-2.206-.242-.579-.487-.501-.669-.51l-.57-.01c-.198 0-.52.074-.792.372s-1.04 1.016-1.04 2.479 1.065 2.876 1.213 3.074c.149.198 2.095 3.2 5.076 4.487.709.306 1.263.489 1.694.626.712.226 1.36.194 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.695.248-1.29.173-1.413z"/></svg></button>
            </div>
          ) : <div className="text-[10px] font-black text-slate-400 bg-slate-50 px-3 h-8 flex items-center rounded-xl border border-slate-100">המספר חסוי</div>}
        </div>
      )}
      ...
      <div className="pt-4 border-t border-slate-100/50">
        ...
        <form onSubmit={handleCommentSubmit} className="relative flex items-center gap-2 mb-3 px-4">
          ...
          <button type="submit" disabled={!commentText.trim()} className="absolute left-5 top-1 bottom-1 w-10 flex items-center justify-center bg-[#1D4ED8] text-white rounded-lg transition active:scale-95 disabled:opacity-50 disabled:bg-slate-300"><svg className="w-4 h-4 transform -rotate-45" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2.5"><path strokeLinecap="round" strokeLinejoin="round" d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8"></path></svg></button>
        </form>
        <div className="flex gap-2 overflow-x-auto hide-scrollbar pb-1 px-4">
          {quickReplies.map(reply => (
            <button key={reply} onClick={(e) => { e.stopPropagation(); handleCommentSubmit(e, reply); }} className="whitespace-nowrap px-4 h-8 bg-white hover:bg-slate-50 text-[#1D4ED8] text-[11px] font-black rounded-xl border border-[#1D4ED8]/20 transition-colors active:scale-95 shadow-sm flex items-center justify-center shrink-0">{reply}</button>
          ))}
        </div>
      </div>
    </div>
