        ...
        {openMenuId === item.id && (
          <>
            ...
            <div className="absolute left-0 top-12 w-[210px] bg-white/95 backdrop-blur-xl border border-slate-100 shadow-[0_15px_50px_rgba(0,0,0,0.15)] rounded-2xl z-[150] py-2 animate-in zoom-in-95 overflow-hidden">
              ...
              <button onClick={(e) => { e.stopPropagation(); navigator.clipboard.writeText(`${item.title}\n${item.description || ''}`); onToggleMenu(null); }} className="w-full px-5 h-12 text-sm font-bold text-slate-700 hover:bg-slate-50 flex items-center justify-between border-b border-slate-100/50 transition-colors">
              ...
              <button onClick={(e) => { e.stopPropagation(); window.open(`https://wa.me/?text=${encodeURIComponent(`*${item.title}*\n${item.description || ''}`)}`, '_blank'); onToggleMenu(null); }} className="w-full px-5 h-12 text-sm font-bold text-slate-700 hover:bg-slate-50 flex items-center justify-between border-b border-slate-100/50 transition-colors">
              ...
              {isAdmin && (
                <button onClick={() => onTogglePin(item.id, item.is_pinned)} className="w-full px-5 h-12 text-sm font-bold text-slate-700 hover:bg-slate-50 flex items-center justify-between border-b border-slate-100/50 transition-colors">
                  <span>{item.is_pinned ? 'בטל נעיצה' : 'נעץ אירוע'}</span>
                  <svg className={`w-5 h-5 shrink-0 ${item.is_pinned ? 'text-[#F59E0B] fill-[#F59E0B]' : 'text-[#1D4ED8] fill-current'}`} viewBox="0 0 24 24"><path d="M16 11V5.5L17.5 4V3H6.5V4L8 5.5V11L6 14V15H11V21H13V15H18V14L16 11Z"/></svg>
                </button>
              )}

              {(isAdmin || isOwner) && (
                <>
                  <button onClick={() => onStartEdit(item)} className="w-full px-5 h-12 text-sm font-bold text-slate-700 hover:bg-slate-50 flex items-center justify-between border-b border-slate-100/50 transition-colors">
                  ...
                  {isPackage && onResolveItem && (
                    <button onClick={() => onResolveItem(item.id)} className="w-full px-5 h-12 text-sm font-bold text-slate-700 hover:bg-slate-50 flex items-center justify-between border-b border-slate-100/50 transition-colors">
                      <span>סמן שנאסף</span>
                  ...
                  <button onClick={() => onDelete(item.id)} className="w-full px-5 h-12 text-sm font-bold text-rose-500 hover:bg-red-50 flex items-center justify-between mt-1 pt-1 transition-colors">
                    <span>מחיקה</span>
