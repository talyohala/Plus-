...
interface MarketplaceItemCardProps {
  ...
  openMenuId: string | null; editingItemId: string | null;
  editItemData: any; mainCategories: string[]; isSubmitting: boolean;
...
export default function MarketplaceItemCard({ item, currentUserId, isAdmin, isSaved, openMenuId, editingItemId, editItemData, mainCategories, isSubmitting, onToggleMenu, onToggleSave, onTogglePin, onStartEdit, onCancelEdit, onUpdateEditData, onSubmitEdit, onDelete, onMediaClick, formatWhatsApp, timeFormat }: MarketplaceItemCardProps) {
  ...
  return (
    <div className={`backdrop-blur-xl p-4 rounded-[2rem] border relative overflow-hidden transition-all duration-300 ${item.is_pinned ? 'bg-gradient-to-br from-blue-50 to-white border-[#1D4ED8]/30 shadow-lg' : 'bg-white/90 border-slate-100 shadow-sm'} ${isOpen ? 'z-50' : 'z-10'}`}>
      ...
      <div className="absolute top-0 right-0 flex overflow-hidden rounded-bl-[1.5rem] rounded-tr-[2rem] z-10">
        ...
        {item.is_pinned && <div className="px-3 py-1.5 bg-amber-500 text-white text-[10px] font-black border-r border-amber-400">📌</div>}
      </div>
      ...
      <div className="absolute top-3 left-3 z-50 flex items-center gap-1.5">
        ...
        <button onClick={(e) => { e.stopPropagation(); onToggleMenu(isOpen ? null : item.id); }} className="w-8 h-8 flex items-center justify-center transition hover:scale-110 text-slate-400 hover:text-[#1D4ED8] bg-white/50 rounded-full">
          <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path d="M10 6a2 2 0 110-4 2 2 0 010 4zM10 12a2 2 0 110-4 2 2 0 010 4zM10 18a2 2 0 110-4 2 2 0 010 4z" /></svg>
        </button>

        {isOpen && (
          <div className="absolute left-0 top-8 w-44 bg-white/95 backdrop-blur-xl border border-slate-100 shadow-2xl rounded-2xl z-[150] py-2 overflow-hidden">
            ...
            {isAdmin && (
              <button onClick={() => onTogglePin(item.id, item.is_pinned)} className="w-full text-right px-4 py-3 text-xs font-bold text-slate-700 hover:bg-slate-50 flex items-center gap-2 border-t border-slate-50">
                <svg className="w-4 h-4 shrink-0 text-[#1D4ED8]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M16 4h-8a1 1 0 0 0-1 1v5l-1.5 3.5h11l-1.5-3.5v-5a1 1 0 0 0-1-1z" /><path d="M12 14v7" /></svg>
            ...
            {isOwner && (
              <button onClick={() => onStartEdit(item)} className="w-full text-right px-4 py-3 text-xs font-bold text-slate-700 hover:bg-slate-50 flex items-center gap-2 border-t border-slate-50">
                <svg className="w-4 h-4 text-slate-500 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.536L16.732 3.732z" /></svg>
            ...
            {(isOwner || isAdmin) && (
              <button onClick={() => onDelete(item.id)} className="w-full text-right px-4 py-3 text-xs font-bold text-slate-700 hover:bg-red-50 flex items-center gap-2 border-t border-slate-50">
                <svg className="w-4 h-4 text-red-500 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
      ...
      {editingItemId === item.id ? (
        ...
        <>
          <div className="pt-8 pr-1 flex justify-between items-start">
            ...
            {item.price > 0 && item.category !== 'חבילות ודואר' && item.category !== 'למסירה' && item.category !== 'בקשות שכנים' && (
            ...
            <div className="flex items-center gap-2.5">
               ...
               <div className="flex flex-col">
                 ...
                 <span className="text-[10px] text-slate-500 font-bold">
                    {item.profiles?.apartment ? `דירה ${item.profiles.apartment} • ` : ''}{timeFormat(item.created_at)}
                 </span>
               </div>
            </div>
          ...
          <div className="mt-4 pt-3 border-t border-slate-50">
              ...
              <div className="flex gap-2 overflow-x-auto hide-scrollbar pb-1 mb-3">
                {(isOwner ? ownerReplies : tenantReplies).map((reply, idx) => (
                  <button 
                    ...
                    className="h-8 px-3 bg-white hover:bg-slate-50 text-[#1D4ED8] border border-[#1D4ED8]/20 rounded-xl text-[11px] font-bold shrink-0 transition active:scale-95 shadow-sm"
                  >
                    {reply}
                  </button>
                ))}
            ...
            <form onSubmit={(e) => { e.preventDefault(); if(customNote.trim()) handleAddComment(customNote.trim()); }} className="flex items-center gap-2 bg-white border border-slate-200 rounded-xl px-3 py-1.5 shadow-inner">
              <input 
                ...
                placeholder={isOwner ? "השב לשכנים..." : "הגב על המודעה..."} 
                className="flex-1 bg-transparent text-xs outline-none font-bold text-slate-800 placeholder-slate-400 h-8" 
              />
