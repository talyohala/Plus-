...
export interface MarketplaceItem {
  ...
  created_at: string; user_id: string; item_type?: string; poll_options?: any[];
...
export default function MarketplaceItemCard({ item, currentUserId, isAdmin, isSaved, openMenuId, editingItemId, editItemData, mainCategories, isSubmitting, onToggleMenu, onToggleSave, onTogglePin, onStartEdit, onCancelEdit, onUpdateEditData, onSubmitEdit, onDelete, onMediaClick, formatWhatsApp, timeFormat }: MarketplaceItemCardProps) {
  ...
  const handleVote = async (optionId: string) => {
    ...
    await supabase.from('marketplace_votes').upsert({ item_id: item.id, user_id: currentUserId, option_id: optionId }, { onConflict: 'item_id,user_id' });
    mutateVotes();
  };

  const totalVotes = votes?.length || 0;
  const myVote = votes?.find((v: any) => v.user_id === currentUserId)?.option_id;
        ...
        <form onSubmit={(e) => onSubmitEdit(e, item.id)} className="p-3 flex flex-col gap-3 bg-[#1D4ED8]/5 rounded-2xl mt-8 border border-[#1D4ED8]/10">
          ...
          <div className="flex gap-2">
            <select value={editItemData.category} onChange={(e) => onUpdateEditData({ ...editItemData, category: e.target.value })} className="flex-1 bg-white border border-[#1D4ED8]/20 rounded-xl px-2 py-2.5 text-xs font-bold outline-none shadow-sm text-slate-800">
              {mainCategories.filter(c => c !== 'הכל' && c !== 'שמורים' && c !== 'קהילה' && c !== 'סקרים').map(c => <option key={c} value={c}>{c}</option>)}
            </select>
            {editItemData.category !== 'בקשות שכנים' && editItemData.category !== 'למסירה' && editItemData.category !== 'חבילות ודואר' && !isPoll && (
          ...
          {!isPoll && <input type="tel" required value={editItemData.contact_phone} onChange={(e) => onUpdateEditData({ ...editItemData, contact_phone: e.target.value })} className="w-full bg-white border border-[#1D4ED8]/20 rounded-xl px-3 py-2.5 text-xs font-mono outline-none text-left shadow-sm text-slate-800" dir="ltr" placeholder="050-0000000" />}
          {!isPoll && <textarea value={editItemData.description} onChange={(e) => onUpdateEditData({ ...editItemData, description: e.target.value })} className="w-full bg-white border border-[#1D4ED8]/20 rounded-xl px-3 py-2.5 text-xs font-medium outline-none min-h-[60px] shadow-sm text-slate-800 resize-none" placeholder="תיאור" />}
        ...
        <>
          <div className="pt-8 pr-1 flex justify-between items-start">
            <div className="flex-1">
              <h3 className={`text-[17px] font-black leading-tight mb-1.5 ${item.is_pinned ? 'text-amber-700' : 'text-slate-800'}`}>{item.title}</h3>
              {!isPoll && <p className="text-sm font-medium text-slate-600 mb-3 whitespace-pre-wrap">{item.description}</p>}
            </div>
            {item.price > 0 && !isPoll && !['חבילות ודואר', 'למסירה', 'בקשות שכנים', 'השאלות כלים'].includes(item.category) && (
              <span className={`text-lg font-black shrink-0 pl-1 ${item.is_pinned ? 'text-amber-600' : 'text-[#1D4ED8]'}`}>₪{item.price.toLocaleString()}</span>
            )}
          </div>
          ...
          {isPoll && item.poll_options && (
            <div className="mb-4 space-y-2.5 mt-2">
              {item.poll_options.map((opt: any) => {
                const optVotes = votes?.filter((v: any) => v.option_id === opt.id).length || 0;
              ...
              <div className="text-[10px] font-bold text-slate-400 text-left pt-1 px-1">סה"כ {totalVotes} הצבעות</div>
            </div>
          )}

          {item.media_url && !isPoll && (
          ...
          <div className="flex items-center justify-between pt-4 border-t border-slate-100 mt-2">
            ...
            {!isOwner && item.contact_phone && !isPoll && (
              <div className="flex items-center gap-2 shrink-0">
                <a href={`tel:${item.contact_phone}`} onClick={(e) => e.stopPropagation()} className="w-9 h-9 rounded-xl bg-white border border-slate-100 shadow-sm active:scale-95 transition flex items-center justify-center hover:bg-slate-50">
                ...
                <a href={formatWhatsApp(item.contact_phone)} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="w-9 h-9 rounded-xl bg-[#25D366] text-white hover:bg-[#20bd5a] transition active:scale-95 flex items-center justify-center shadow-sm">
          ...
          <div className="mt-4 pt-3 border-t border-slate-50">
            ...
            {!isPoll && (isPackage || isRequest || isOwner) && (
