...
export default function MarketplaceItemCard({ item, currentUserId, isAdmin, isSaved, openMenuId, editingItemId, editItemData, isSubmitting, onToggleMenu, onToggleSave, onTogglePin, onStartEdit, onCancelEdit, onDelete, onMediaClick, onResolveItem, onAddComment, onVote, formatWhatsApp, timeFormat }: Props) {
  ...
  const totalVotes = localCounts.yes + localCounts.no;
  const yesPercent = totalVotes > 0 ? Math.round((localCounts.yes / totalVotes) * 100) : 0;
  const noPercent = totalVotes > 0 ? Math.round((localCounts.no / totalVotes) * 100) : 0;
        ...
        {openMenuId === item.id && (
          <>
            ...
            <div className="absolute left-0 top-12 w-[210px] bg-white/95 backdrop-blur-xl border border-slate-100 shadow-[0_15px_50px_rgba(0,0,0,0.15)] rounded-2xl z-[150] py-2 animate-in zoom-in-95 overflow-hidden">
              ...
              {(isAdmin || isOwner) && (
                <>
                  <button onClick={() => onStartEdit(item)} className="w-full text-right px-5 h-12 text-sm font-bold text-slate-700 hover:bg-slate-50 flex items-center justify-start gap-4 transition-colors border-b border-slate-100/50">
                    <svg className="w-5 h-5 text-slate-500 shrink-0" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                    <span>עריכת פרטים</span>
                  </button>
                  {isPackage && onResolveItem && (
                    <button onClick={() => onResolveItem(item.id)} className="w-full text-right px-5 h-12 text-sm font-bold text-slate-700 hover:bg-slate-50 flex items-center justify-start gap-4 transition-colors border-b border-slate-100/50">
                      <svg className="w-5 h-5 text-slate-500 shrink-0" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7"></path></svg>
                      <span>סמן שנאסף</span>
                    </button>
                  )}
                  <button onClick={() => onDelete(item.id)} className="w-full text-right px-5 h-12 text-sm font-bold text-rose-500 hover:bg-red-50 flex items-center justify-start gap-4 mt-1 pt-1 transition-colors">
                    <svg className="w-5 h-5 text-rose-500 shrink-0" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                    <span>מחיקה</span>
                  </button>
                </>
              )}
