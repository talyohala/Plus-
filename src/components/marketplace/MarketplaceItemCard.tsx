...
export default function MarketplaceItemCard({ item, currentUserId, isAdmin, isSaved, openMenuId, editingItemId, editItemData, isSubmitting, onToggleMenu, onToggleSave, onTogglePin, onStartEdit, onCancelEdit, onUpdateEditData, onSubmitEdit, onDelete, onMediaClick, onResolveItem, onAddComment, onVote, formatWhatsApp, timeFormat }: Props) {
  ...
  const isPending = item.status === 'pending_approval' || (item.status === 'frozen' && item.title.startsWith('[הזמנה]'));
  ...
  return (
    <div className={`bg-white/90 backdrop-blur-xl rounded-[2rem] pt-5 pb-4 relative transition-all duration-300 overflow-hidden ${item.is_pinned ? 'border border-orange-200/60 bg-gradient-to-br from-orange-50/80 to-white shadow-[0_8px_25px_rgba(249,115,22,0.15)]' : 'border border-slate-100 shadow-[0_8px_30px_rgba(29,78,216,0.04)]'} ${openMenuId === item.id ? 'z-50' : 'z-10'}`} dir="rtl">
        ...
        <div className="mt-4 px-4 mb-4 flex flex-col gap-3">
          <button onClick={(e) => { e.stopPropagation(); if (!isPast && !isFrozen) onVote(item.id, 'yes'); }} className={`relative w-full h-14 rounded-2xl overflow-hidden transition-all duration-300 border ${myVote === 'yes' ? 'border-[#10B981] shadow-md scale-[0.98]' : 'border-slate-200 bg-slate-50 hover:bg-slate-100 shadow-sm'} ${isPast || isFrozen ? 'cursor-default opacity-80' : 'active:scale-95'}`}>
            ...
            <div className="absolute inset-0 flex items-center font-black pointer-events-none">
              {myVote === 'yes' && <div className="absolute right-4 w-6 h-6 bg-[#10B981] rounded-full flex items-center justify-center text-white shadow-sm"><svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="3" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7"></path></svg></div>}
          ...
          <button onClick={(e) => { e.stopPropagation(); if (!isPast && !isFrozen) onVote(item.id, 'no'); }} className={`relative w-full h-14 rounded-2xl overflow-hidden transition-all duration-300 border ${myVote === 'no' ? 'border-rose-500 shadow-md scale-[0.98]' : 'border-slate-200 bg-slate-50 hover:bg-slate-100 shadow-sm'} ${isPast || isFrozen ? 'cursor-default opacity-80' : 'active:scale-95'}`}>
            ...
            <div className="absolute inset-0 flex items-center font-black pointer-events-none">
              {myVote === 'no' && <div className="absolute right-4 w-6 h-6 bg-rose-500 rounded-full flex items-center justify-center text-white shadow-sm"><svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="3" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7"></path></svg></div>}
      ...
      {/* מחיר ויצירת קשר הועברו למטה! בשורה אחת לפני התגובות */}
      {!isPoll && (
        <div className="flex justify-between items-center mb-4 px-4 mt-2">
          ...
          {/* אזור יצירת קשר - רקע כחול וירוק מלא */}
          {showContact ? (
      ...
      {/* אזור התגובות הפנימיות עם גלילה (מקסימום 3 במסך) */}
