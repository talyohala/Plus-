...
export default function TicketCard({
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'טופל': return { text: 'טופל', style: 'bg-emerald-500 text-white' };
  ...
  return (
    <div className={`bg-white/90 backdrop-blur-xl p-4 pt-8 rounded-[2rem] shadow-sm border transition-all relative ${
      ...
      <div className="absolute top-0 right-0 flex overflow-hidden rounded-bl-[1.5rem] rounded-tr-[2rem] z-10">
        {ticket.is_pinned ? (
          <div className="px-4 py-1.5 bg-amber-500 text-white text-[10px] font-black uppercase tracking-wider shadow-sm">נעוץ</div>
        ) : (
          <div className={`px-4 py-1.5 font-black text-[10px] shadow-sm ${badge.style}`}>{badge.text}</div>
        )}
      </div>
