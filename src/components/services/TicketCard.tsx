...
export default function TicketCard({
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'בטיפול': return { text: 'בטיפול', style: 'bg-amber-500 text-white' };
  ...
  return (
    <div className={`bg-white/90 backdrop-blur-xl p-4 pt-8 rounded-[2rem] shadow-sm border transition-all relative overflow-hidden ${
      ticket.is_pinned ? 'border-orange-200 bg-gradient-to-br from-orange-50 to-white shadow-[0_8px_20px_rgba(249,115,22,0.15)]' : 'border-[#1D4ED8]/10'
      ...
      <div className="flex gap-3">
        ...
        <div className="flex-1 py-0.5">
          <h3 className={`font-black text-sm mb-1 ${ticket.is_pinned ? 'text-orange-600' : 'text-slate-800'}`}>{ticket.title}</h3>
